...
const devopsArticles = course('devops', [
  ...
  item('第三部分：模型推理服务', 'transformer-inference-lifecycle', '大模型怎样生成一个 Token？从分词、Prefill 到 Decode', '从 Token 与 Tokenizer 开始，解释文本怎样经过 Chat Template、Embedding、Transformer、Prefill、Decode、采样、停止和 Detokenize 变成输出。', ['Token', 'Prefill', 'Decode'], ['理解模型制品、Tokenizer 和 Serving', '了解矩阵是数字的二维排列'], ['用自己的话解释一个 Token 从输入到输出的计算顺序', '区分 Prefill、Decode、采样、停止和文本解码'], '完成一次单请求 Token 生成的解释性推演', ['输入、状态变化、输出与停止证据完整', 'Token 边界不会被误写成汉字、单词或网络分块边界'], 'official', 'walkthrough'),
  item('第三部分：模型推理服务', 'continuous-batching-kv-cache', 'Continuous Batching 和 KV Cache 是什么？推理服务怎样同时处理多个请求', '从静态 Batch 开始，解释 Continuous Batching、KV Cache、Block、PagedAttention、抢占与公平性，并用计算过程估算并发显存。', ['Continuous Batching', 'KV Cache', 'PagedAttention'], ['理解 Prefill、Decode 与 Token 生成', '了解显存用于保存模型运行状态'], ['解释请求怎样在推理迭代间动态加入和退出', '按模型结构与 Token 数估算 KV Cache 并发边界'], '完成一次三请求动态调度和缓存分配推演', ['Batch、Sequence、Token、Block 与请求数不会混用', '吞吐优化的尾延迟和公平代价被说明'], 'official', 'walkthrough'),
  item('第三部分：模型推理服务', 'vllm-openai-compatible-serving', 'vLLM 是什么？怎样启动、调用并诊断一个模型服务', '从 vLLM 的引擎与 OpenAI 兼容 Server 开始，解释环境、模型加载、调度、就绪、API、取消和故障诊断，并给出可静态检查的启动流程。', ['vLLM', 'OpenAI Compatible API', 'Model Serving'], ['理解 LLM Serving、Prefill、Decode 和 KV Cache', '会使用 Python 虚拟环境与 curl'], ['解释 vLLM Engine、Scheduler 与 API Server 的责任', '从环境、制品、加载、就绪、请求和资源证据诊断服务'], '完成一个 vLLM 服务的静态配置与调用推演', ['命令参数按目标版本核对且不宣称未执行的 GPU 结果', '失败能定位到环境、加载、准入、调度或响应阶段'], 'official-guided-operation', 'implementation'),
  item('第三部分：模型推理服务', 'model-artifacts-precision-quantization', '模型精度与量化是什么？FP16、BF16、INT8 和 INT4 改变了什么', '从有限位数表示数值开始，解释 FP32、FP16、BF16、INT8、INT4、权重量化、激活量化、校准、Kernel、显存与质量验证。', ['Precision', 'Quantization', 'Model Artifact'], ['理解模型权重、推理引擎和 KV Cache', '会做字节与容量的基本计算'], ['解释浮点精度与整数量化各自改变的数据表示', '计算权重内存下限并判断量化兼容与质量边界'], '完成一个模型精度与量化候选的静态审查', ['权重、激活、KV Cache 和累加精度不会混用', '未运行的质量与性能结果不被写成事实'], 'official', 'decision'),
  item('第四部分：GPU 与 CUDA', 'gpu-computing-foundations', 'GPU 是什么？为什么 AI 模型适合在 GPU 上计算', '从 CPU、GPU、并行计算和矩阵运算四个基础概念开始，解释 AI 模型为何受益于 GPU 的计算吞吐与内存带宽，并说明适用边界。', ['GPU', 'CPU', 'Parallel Computing'], ['会做基本乘加运算', '理解程序、进程与模型推理'], ['分别解释 CPU、GPU、并行计算和矩阵乘法', '判断一段 AI 工作能否从 GPU 并行和内存带宽受益'], '完成一次矩阵乘从 CPU 提交到 GPU 返回的解释性推演', ['GPU 不被简化成“核心多所以一定快”', '传输、分支、任务规模和不支持算子边界被说明'], 'official', 'walkthrough'),
  item('第四部分：GPU 基础', 'cuda-programming-model', 'CUDA 是什么？Kernel、Thread、Block、Warp 和 SM 如何协作', '从 CUDA、Host 和 Device 的分工开始，解释 Kernel、Thread、Block、Warp 与 SM 的执行关系、内存层级、同步和失败证据。', ['CUDA', 'Kernel', 'SM'], ['理解 GPU、CPU、并行计算和矩阵运算', '会阅读简单的 C 或 Python 代码'], ['解释 CUDA 执行模型和线程层级', '沿一个 Kernel 推演输入、调度、输出与错误'], '完成一张 CUDA 执行映射图', ['Thread、Block、Warp 和 SM 的所有权关系正确', '没有真实 GPU 时明确标记为静态检查或机制推演'], 'official', 'walkthrough'),
  item('第四部分：GPU 基础', 'gpu-cuda-vram-nvidia-smi', '显存是什么？权重、激活、工作区与 KV Cache 为什么会导致 OOM', '从显存和 HBM 的定义开始，拆解权重、激活、工作区、KV Cache、分配器与 CUDA 上下文的占用，并推演一次 OOM 诊断。', ['GPU Memory', 'CUDA', 'OOM'], ['理解 GPU 与 CUDA 执行模型', '理解模型精度和 KV Cache'], ['建立显存组成账本', '按加载、运行、突发和碎片证据诊断 OOM'], '完成一张 GPU 预检与显存估算表', ['Driver、Runtime 与显存容量没有混为一谈', '未在真实 GPU 执行的数值只作为解释性估算'], 'official', 'diagnosis'),
  item('第五部分：Kubernetes AI Infra', 'kubernetes-ai-platform-basics', 'Kubernetes 是什么？它怎样让容器服务保持在期望状态', '从声明式期望状态与调谐开始，解释控制面、Node、Pod、Deployment、Service、探针、配置、存储和一次发布过程。', ['Kubernetes', 'Pod', 'Deployment'], ['理解 Linux 进程、网络和容器', '会阅读 YAML'], ['解释 Kubernetes 的调谐模型和核心对象', '沿对象状态定位模型服务为何没有接到流量'], '完成一张 AI 工作负载对象图', ['对象之间的控制与选择关系明确', 'Kubernetes 不被描述为理解模型语义'], 'official', 'walkthrough'),
  item('第五部分：Kubernetes AI Infra', 'kubernetes-ai-service-deployment', 'GPU 怎样进入 Kubernetes Pod？从节点驱动到模型就绪', '沿节点硬件、驱动、Container Toolkit、Device Plugin、扩展资源、容器 Runtime、模型制品和探针解释 GPU Pod 的完整启动链。', ['Kubernetes', 'GPU Operator', 'Device Plugin'], ['理解 Kubernetes 核心对象', '理解 CUDA、显存和模型加载'], ['解释 GPU 能力进入 Pod 的路径', '设计模型加载、Warmup、就绪和终止流程'], '完成一份 GPU 模型服务部署清单推演', ['探针不会把加载中的实例送入流量', '示例标明静态检查和真实集群验证边界'], 'official', 'walkthrough'),
  item('第五部分：Kubernetes AI Infra', 'kubernetes-gpu-scheduling', 'Kubernetes 怎样调度 GPU？整卡、共享、MIG 与扩缩容如何选择', '从扩展资源和节点标签开始，解释整卡、GPU 型号、显存、拓扑、MIG、时间共享、MPS、队列与扩缩容信号。', ['Kubernetes', 'GPU Scheduling', 'MIG'], ['理解 Kubernetes 调度、GPU Device Plugin 和显存账本', '理解推理请求、队列和 KV Cache'], ['为不同模型选择 GPU 调度和共享方式', '从请求压力而不是单一 GPU 利用率设计扩缩容'], '完成一张 GPU 调度决策表', ['设备数量、显存、型号和拓扑边界不混淆', '未在真实集群验证的结论明确标记'], 'official', 'decision'),
  item('第六部分：模型平台与 Agent Runtime', 'llm-gateway-design', 'LLM Gateway 是什么？一个模型请求怎样完成鉴权、路由和计费', '从 Gateway 的边界开始，解释 API 鉴权、模型路由、配额、重试、流式取消、计费、审计和故障降级。', ['LLM Gateway', 'API Security', 'Routing'], ['理解 HTTP、SSE、LLM Serving 和 Kubernetes Service', '会阅读 JSON 配置'], ['解释 Gateway 与模型服务的职责边界', '推演一次请求从鉴权到计费、失败和退款'], '完成一份多模型 Gateway 路由与计费设计', ['外部身份、内部模型请求和账务状态分开', '流式取消与重试不会重复扣费'], 'official-guided-operation', 'walkthrough'),
  item('第六部分：模型平台与 Agent Runtime', 'multi-model-platform', '多模型平台是什么？模型版本、部署实例和流量切换怎样管理', '从模型制品、版本、部署实例和逻辑别名的区别开始，解释发布、灰度、容量、配置、回滚、审计和生命周期管理。', ['Model Platform', 'Model Registry', 'Release'], ['理解 LLM Gateway、模型制品和 Kubernetes Deployment', '会阅读版本与配置记录'], ['区分模型版本、部署实例、路由别名和运行状态', '设计一次可回滚的多模型发布流程'], '完成一张模型版本到流量的关系图', ['版本身份、实例状态和账务引用可追溯', '灰度与回滚不依赖修改客户端代码'], 'official-guided-operation', 'architecture'),
  item('第六部分：模型平台与 Agent Runtime', 'agent-runtime-infrastructure', 'Agent Runtime 是什么？模型、工具、状态、取消与恢复怎样协作', '从 Agent Runtime 的职责开始，解释模型调用、工具契约、状态机、执行循环、权限、取消、重试、持久化和恢复。', ['Agent Runtime', 'Tool Calling', 'State'], ['理解 LLM Gateway、队列、数据库和基础 Python', '知道模型输出可能是不确定的文本或工具调用'], ['区分模型、Runtime、工具和外部系统的责任', '推演一次 Agent 任务的执行、取消、重试和恢复'], '完成一份可恢复 Agent Runtime 状态图', ['工具调用有权限、超时和幂等边界', 'Runtime 重启不会重复执行不可逆副作用'], 'official-guided-operation', 'walkthrough'),
  item('第六部分：模型平台与 Agent Runtime', 'rag-infrastructure', 'RAG Infra 是什么？文档怎样变成可检索且有权限的知识', '从文档身份、解析、切片、Embedding、索引、权限过滤、检索、重排、引用、更新和删除解释 RAG 基础设施。', ['RAG', 'Vector Search', 'Ingestion'], ['理解对象存储、PostgreSQL、Embedding 和 Agent 工具调用', '知道用户数据需要权限隔离'], ['解释文档从上传到检索结果的状态链', '设计不会越权、可更新和可删除的 RAG 基础设施'], '完成一份带权限过滤的文档索引推演', ['检索结果只来自当前用户可见范围', '更新与删除能追踪到 Chunk、向量和缓存'], 'official-guided-operation', 'walkthrough'),
  item('第六部分：模型平台与 Agent Runtime', 'ai-observability-slo', 'AI 可观测性是什么？Trace、Metric、Log、质量和成本怎样关联', '从可观测性的定义开始，解释请求标识、Trace、Metric、Log、质量、成本、SLO、隐私、告警和一次故障诊断。', ['Observability', 'OpenTelemetry', 'SLO'], ['理解 Gateway、Serving、Agent Runtime 和 RAG 的请求链', '知道延迟分位数、错误率和日志基础'], ['把用户请求与模型、工具、检索、GPU 和账务证据关联', '设计同时覆盖运行、质量、成本和隐私的 SLO'], '完成一条 AI 请求的可观测性推演', ['不用单一 GPU 利用率或平均延迟代表用户体验', 'Prompt 和输出不会默认进入高权限不足的日志'], 'official-guided-operation', 'diagnosis'),
  item('第六部分：模型平台与 Agent Runtime', 'ai-capacity-load-cost', '容量规划是什么？怎样从请求、队列和延迟估算 GPU 与成本', '从工作负载、吞吐、并发、排队和延迟开始，使用 Little 定律、Token 分布、压测与余量估算 GPU 副本和单位成本。', ['Capacity Planning', 'Queueing', 'GPU Cost'], ['理解 TTFT、TPOT、Serving 队列、Batch 和 GPU 调度', '会做基础比例、分位数和单位换算'], ['把请求分布转换成 Token 工作量和副本需求', '区分理论估算、候选压测和生产余量'], '完成一份 AI Serving 容量与成本计算', ['不用平均请求或峰值 FLOPS 直接承诺容量', '队列、取消、冷启动和故障余量进入计算'], 'official-guided-operation', 'calculation'),
  item('第六部分：模型平台与 Agent Runtime', 'ai-platform-security', 'AI 平台需要防什么？多租户、Secret、数据与工具调用的安全边界', '从资产、主体和信任边界开始，解释多租户隔离、身份、Secret、数据、模型制品、提示注入、工具沙箱、网络、日志和事件响应。', ['AI Security', 'Multi-tenancy', 'Secrets'], ['理解 Gateway、RAG、Agent Runtime、Serving 与 Kubernetes', '知道最小权限、网络隔离和审计基础'], ['为 AI 平台画出主体、资产和信任边界', '设计从请求到工具、数据和模型的确定性安全检查'], '完成一份多租户 AI 请求威胁推演', ['Prompt 不会被当成授权来源', '越权、Secret 泄露和资源滥用有明确失败与恢复证据'], 'official-guided-operation', 'threat-model'),
  item('第七部分：分布式训练与交付', 'distributed-training-infrastructure', '分布式训练是什么？为什么一张 GPU 不够时需要并行策略', '从训练状态与单卡限制开始，解释数据并行、张量并行、流水线并行、序列并行、同步、检查点、调度和故障恢复。', ['Distributed Training', 'Parallelism', 'Checkpoint'], ['理解 GPU、显存、矩阵运算和神经网络训练基础', '知道前向、反向、梯度和优化器的基本含义'], ['区分常见并行策略解决的资源问题', '推演一次多 GPU 训练步骤和失败恢复'], '完成一张并行训练状态与通信图', ['计算、模型状态和通信开销没有混为一谈', '未在真实多卡集群执行的结果只作为机制推演'], 'official-guided-operation', 'walkthrough'),
  item('第七部分：分布式训练与交付', 'deepspeed-zero-infrastructure', 'DeepSpeed ZeRO 是什么？参数、梯度和优化器状态怎样分片', '从训练显存账本开始，解释 ZeRO Stage 1、2、3、参数聚合、通信、Offload、初始化、配置、检查点与恢复。', ['DeepSpeed', 'ZeRO', 'Sharding'], ['理解数据并行、参数、梯度、优化器和 Collective', '理解训练 Checkpoint 与 GPU/CPU 内存'], ['区分 ZeRO 三个阶段分片的状态', '推演一次参数聚合、反向分片和检查点恢复'], '完成一份 ZeRO 分片状态与配置推演', ['显存节省与通信、CPU、I/O 代价同时说明', '未在真实多卡环境运行的配置只做静态检查'], 'official-guided-operation', 'walkthrough'),
  item('第七部分：分布式训练与交付', 'nccl-gpu-communication', 'NCCL 是什么？多张 GPU 怎样完成 AllReduce 与跨机通信', '从 Collective 和通信组开始，解释 AllReduce、Reduce-Scatter、AllGather、Ring、Tree、拓扑、传输、初始化、故障与基准测试。', ['NCCL', 'AllReduce', 'RDMA'], ['理解分布式训练、Rank、梯度和 GPU 拓扑', '知道 TCP、PCIe、NVLink 与基本网络概念'], ['解释常见 Collective 的输入输出与所有权', '沿一次跨机 AllReduce 定位初始化、拓扑和传输故障'], '完成一份 NCCL 通信链与故障推演', ['网络超时不会掩盖更早的 Rank OOM 或退出', '未在真实集群执行的带宽和拓扑结论不写成实测'], 'official-guided-operation', 'diagnosis'),
  item('第八部分：交付与综合项目', 'ci-cd-artifact-security', 'AI 系统的 CI/CD 是什么？代码、模型和配置怎样成为可信制品', '从提交到已验证镜像，串联依赖锁定、测试、制品、SBOM、签名、证明、Secret 和环境提升。', ['CI/CD', 'Supply Chain', 'SBOM'], ['Git、容器和模型制品基础'], ['设计 AI 平台构建流水线', '让代码、模型和配置版本可追溯'], '完成一份不可变制品发布流程', ['同一制品在环境间提升', 'Secret 不进入镜像、SBOM 或日志'], 'official', 'implementation'),
  item('第八部分：交付与综合项目', 'candidate-migration-rollback-recovery', '发布、迁移、回滚与恢复有什么区别？候选版本怎样安全上线', '沿预检、备份、候选实例、兼容迁移、旁路验证、流量切换、即时回滚和隔离恢复设计发布。', ['Deployment', 'Migration', 'Recovery'], ['理解数据库、容器、Kubernetes 和 CI/CD'], ['设计不中断依赖的低风险发布', '区分应用回滚、数据回退和灾难恢复'], '完成一份发布与恢复 Runbook', ['旧版本在切流时仍可用', '恢复能力由隔离演练而非备份文件证明'], 'anonymized-practice', 'implementation'),
  item('第八部分：交付与综合项目', 'enterprise-ai-platform-capstone', '企业 AI 平台是什么？怎样把 Gateway、Agent、RAG、Serving 与 GPU 连成系统', '把 Gateway、Agent、RAG、vLLM、GPU、PostgreSQL、Redis、对象存储、观测、成本和发布连接成可演进平台。', ['AI Platform', 'Architecture', 'Capstone'], ['完成前 36 章'], ['给出企业 AI 平台的模块、数据和运行边界', '制定从单体到多集群的建设顺序'], '完成一份 Enterprise AI Platform 设计包', ['正常、过载、取消、发布和恢复路径完整', '每项能力都有所有者、证据和停止条件'], 'anonymized-practice', 'decision')
])
