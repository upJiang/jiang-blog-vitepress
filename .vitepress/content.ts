...
export const sections: SectionMeta[] = [
  ...
  { key: 'frontend', title: '前端', description: '保留算法与重学前端课程，并通过可观察页面和最小实现学习现代前端工程。', path: '/docs/frontend/' },
...
const aiAgentArticles = course('ai-agent', [
  item('第一部分：认识模型与 Agent', 'llm-workflow-rag-agent', 'LLM、工作流、RAG 和 Agent 到底有什么区别', '从同一个资料问答任务出发，判断什么时候直接调用模型、什么时候使用工作流、RAG 或 Agent。', ['LLM', 'RAG', 'Agent'], ['会调用普通 HTTP API'], ['画出四种方案的执行路径', '根据任务的不确定性选择方案'], '完成一张 AI 应用选型表', ['能解释四种方案的输入、控制者与输出', '能为三个任务选择实现方式'], 'official', 'decision'),
  item('第一部分：认识模型与 Agent', 'messages-tokens-context', '消息、Token、上下文窗口与模型输入输出', '拆开一次聊天请求，理解消息角色、Token 预算、上下文窗口、采样和停止条件。', ['Token', 'Context', 'Message'], ['了解 JSON', '读过第 1 章'], ['估算一次请求的上下文组成', '识别上下文超限与输出截断'], '检查一份聊天请求的 Token 预算', ['能标出系统消息、历史、工具结果和用户输入', '能说明超限时先裁剪什么'], 'official', 'diagnosis'),
  item('第一部分：认识模型与 Agent', 'structured-output-model-boundaries', '结构化输出、模型边界与确定性程序', '让模型负责语义判断，让程序负责权限、金额、状态和格式校验。', ['Structured Output', 'Schema'], ['了解 JSON Schema', '读过第 2 章'], ['设计结构化输出契约', '区分概率判断与确定性规则'], '为意图识别结果设计 Schema 与校验流程', ['合法输出通过校验', '缺字段和越界值被拒绝'], 'official', 'implementation'),
  item('第二部分：构建 Agent Runtime', 'agent-lifecycle', 'Agent 从收到问题到产生答案经历了什么', '沿一次只读知识问答，理解目标、计划、工具、观察、回答、验证和停止条件。', ['Agent', 'Runtime'], ['读过前 3 章'], ['复述 Agent 完整生命周期', '识别普通工作流与 Agent 循环'], '手工推演一次工具调用循环', ['能写出每一步输入和输出', '能指出循环的停止条件'], 'anonymized-practice'),
  item('第二部分：构建 Agent Runtime', 'agent-framework-selection', '常见 Agent 框架及 LangGraph 选型', '比较 OpenAI Agents SDK、LangGraph、AutoGen、CrewAI、Semantic Kernel 与 Dify 的抽象层和适用任务。', ['LangGraph', 'Framework'], ['读过第 4 章', '了解 Python 函数'], ['根据控制权、状态和部署要求选框架', '说明状态图适合什么任务'], '完成一张框架选型决策表', ['同一需求能比较至少三种实现', '结论包含团队与运行约束'], 'official', 'decision'),
  item('第二部分：构建 Agent Runtime', 'langgraph-state-runtime', 'LangGraph 的 State、Node、Edge、Reducer 与 Checkpoint', '把串行函数改成可观察的状态图，再加入条件边、并行合并和按需恢复。', ['LangGraph', 'State Graph'], ['Python 类型提示', '读过第 5 章'], ['读懂状态图执行顺序', '设计不会互相覆盖的 Reducer'], '画出并推演一张最小状态图', ['正常分支到达终态', '并行结果按 Reducer 合并'], 'anonymized-practice', 'implementation'),
  item('第二部分：构建 Agent Runtime', 'tool-calling-contracts', 'Tool Calling：定义、选择、执行和校验工具', '从只读搜索工具开始，讲清 Schema、白名单、超时、错误和工具返回值为什么都要校验。', ['Tool Calling', 'JSON Schema'], ['读过第 3、4 章'], ['写出工具契约', '验证模型生成的工具参数'], '实现一个受控的只读搜索工具', ['非法工具名被拒绝', '超时和空结果有明确语义'], 'anonymized-practice', 'implementation'),
  item('第二部分：构建 Agent Runtime', 'mcp-skills-subagents', 'MCP、Skill 与 SubAgent 的职责边界', '从一次能力复用需求出发，区分协议连接、知识说明和任务委派。', ['MCP', 'Skill', 'SubAgent'], ['读过第 7 章'], ['解释三种机制解决的问题', '设计只读 MCP 的信任边界'], '为一个资料查询能力选择封装方式', ['能说明连接生命周期', '能指出不可信返回值的校验位置'], 'official', 'decision'),
  item('第三部分：让 Agent 使用知识', 'rag-ingestion-pipeline', 'RAG 数据导入：解析、OCR、清洗与语义切片', '从一份混合文档开始，保留标题、段落、列表、表格与代码，并只对需要的页面做 OCR。', ['RAG', 'OCR', 'Chunking'], ['理解文件和文本编码', '读过第 1 章'], ['设计可重建的数据导入链', '用覆盖率检查内容丢失'], '完成一份文档的解析与切片设计', ['扫描页进入 OCR 分支', '切片可追溯到原文位置'], 'anonymized-practice', 'implementation'),
  item('第三部分：让 Agent 使用知识', 'embedding-vector-space', 'Embedding：从分词、向量空间到相似度计算', '通过三个句子的相似度实验，讲清维度、归一化、余弦、点积、批处理和模型兼容。', ['Embedding', 'Vector'], ['了解数组和基础代数', '读过第 9 章'], ['计算余弦相似度', '识别向量模型与数据版本不兼容'], '完成一个小型相似度与召回实验', ['手算结果与程序输出一致', '更换模型时触发重新向量化判断'], 'official', 'implementation'),
  item('第三部分：让 Agent 使用知识', 'pgvector-index-recall', 'pgvector、索引结构、召回率与向量写入', '从精确扫描推进到 HNSW/IVFFlat，理解距离算子、过滤顺序、索引参数、批量写入和召回评测。', ['PostgreSQL', 'pgvector'], ['SQL 基础', '读过第 10 章'], ['为向量列选择距离与索引', '建立 Recall@K 检查'], '设计一张可版本化的向量表与查询', ['查询使用兼容算子', '候选结果能与精确基线比较'], 'official-guided-operation', 'implementation'),
  item('第三部分：让 Agent 使用知识', 'hybrid-retrieval-rerank', '精确、全文、向量、结构化检索与重排', '用专有名词、同义表达和表格问题解释多路召回、RRF 融合、重排、缓存和降级。', ['Retrieval', 'Rerank', 'RRF'], ['读过第 10、11 章'], ['为不同查询选择召回通道', '解释融合与重排的职责'], '手工合并三路候选列表', ['融合不会因分数尺度不同失真', '降级后仍保留可解释证据'], 'anonymized-practice', 'implementation'),
  item('第三部分：让 Agent 使用知识', 'context-memory-compression', '上下文压缩、短期记忆、摘要与长期记忆', '在固定 Token 预算内组织近期对话、滚动摘要、检索证据和用户可控记忆。', ['Context Engineering', 'Memory'], ['读过第 2、4 章'], ['设计上下文预算表', '区分会话状态、摘要与长期事实'], '裁剪一段超长对话并保留决策依据', ['预算不超限', '敏感信息不会自动进入长期记忆'], 'anonymized-practice', 'implementation'),
  item('第四部分：质量、安全与交付', 'claims-evidence-citations', 'Claim、Evidence、引用生成与答案验证', '把答案拆成可验证 Claim，让每个事实绑定用户可见证据，并对缺证据结论做有限修复。', ['Claim', 'Evidence', 'Citation'], ['读过第 12 章'], ['建立 Claim 与证据的对应关系', '区分回答生成和事实验证'], '审核一份带引用的答案', ['所有事实 Claim 有可见证据', '引用范围和原文位置一致'], 'anonymized-practice', 'diagnosis'),
  item('第四部分：质量、安全与交付', 'agent-security-eval-observability', '权限、提示注入、Eval、Trace、成本和可靠性', '把权限过滤、注入防护、离线评测、执行轨迹、Deadline、模型路由和成本预算放进同一条治理链。', ['Security', 'Evaluation', 'Observability'], ['读过前 14 章'], ['建立 Agent 发布门禁', '定位质量、延迟和成本问题'], '为一次 Agent 版本变更设计评测与观测', ['权限与注入用例被覆盖', '质量和成本指标可关联到版本'], 'anonymized-practice', 'decision'),
  item('第四部分：质量、安全与交付', 'knowledge-agent-capstone', '从零串起一个只读知识 Agent 完整案例', '把请求、会话、检索、工具、证据、流式事件、任务恢复、评测和观测串成完整系统。', ['Agent', 'RAG', 'LangGraph'], ['完成前 15 章'], ['画出完整执行链', '按模块实现并验证只读知识 Agent'], '完成一份匿名知识 Agent 的实现蓝图与验收清单', ['正常问答、无证据、无权限、取消和恢复均有结果', '每项现有能力与演进建议已区分'], 'anonymized-practice', 'implementation')
])
