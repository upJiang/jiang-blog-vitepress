...
export const aiAgentCurriculum: AiAgentArticleSpec[] = [
  ...stage('foundations', [
    ...
    article('messages-tokens-context', 'Message、Token 与上下文：模型一次调用看到了什么', '用同一个问题的三次输入对照，厘清 Context、Message、Token 与 Context Window 的关系，并建立可回放的输入检查方法。', ['Message', 'Token', 'Context', 'Context Window'], 'concept', ['official-model-input', 'kb-context-assembly'], ['llm-workflow-rag-agent']),
    article('python-openai-responses-first-call', '用 Python 调用一次 Responses API', '从同一份请求语义出发，观察 Response、流式事件和 SDK 异常，再把一次调用归档为可判断的结果。', ['Python', 'OpenAI', 'Responses API'], 'implementation', ['official-responses-api', 'example-model-adapter'], ['messages-tokens-context']),
    article('structured-output-model-boundaries', '结构化输出约束格式，不证明业务事实', '从一次搜索候选看清 Structured Outputs、JSON Schema 与 Pydantic 的责任边界，以及格式正确的对象为何仍不能直接执行。', ['Structured Outputs', 'JSON Schema', 'Pydantic'], 'concept', ['official-structured-output', 'kb-runtime-contracts'], ['python-openai-responses-first-call']),
    article('agent-essence-autonomy-boundaries', '什么是 AI Agent：自主性从哪里来，责任边界怎么划分', '用同一项远程访问任务区分固定工作流与反馈决策，说明 Agent 的自主性来自哪些选择，以及模型、运行时、工具和审批者各自负责什么。', ['Agent', 'Autonomy', 'Runtime'], 'concept', ['wl-01-agent-essence', 'kb-runtime-boundaries'], ['structured-output-model-boundaries']),
  ...
  ...stage('tools', [
    article('tool-calling-contracts', 'Tool Calling 的提议、校验与执行契约', '沿一次工具请求拆开目录、模型候选、Schema、可信授权、执行回执和未知副作用。', ['Tool Calling', 'Contract', 'Authorization'], 'implementation', ['wl-03-tool-calling', 'kb-tool-contracts'], ['agent-fit-deterministic-workflow']),
    article('parallel-tool-execution', '并行工具执行怎样保持身份、顺序与预算', '从并行准入到结果融合，处理工具调用的身份归属、语义顺序、共享预算、取消和部分失败。', ['Tool Calling', 'Concurrency', 'Budget'], 'implementation', ['wl-44-parallel-tools', 'kb-parallel-retrieval'], ['tool-calling-contracts']),
