...
export const aiAgentCurriculum: AiAgentArticleSpec[] = [
  ...stage('foundations', [
    ...
    article('messages-tokens-context', 'Message、Token、Context 与 Context Window：模型输入如何组织、计量和限容', '用同一个 OpenAI Responses 请求拆开 Message、Token、Context 与 Context Window，说明输入如何装配、Token 如何计数，以及容量和回放证据的边界。', ['Message', 'Token', 'Context', 'Context Window'], 'concept', ['official-model-input', 'kb-context-assembly'], ['llm-workflow-rag-agent']),
    article('python-openai-responses-first-call', 'Python 调用 Responses API：一次请求如何变成可判断结果', '沿同一份请求观察 Responses API 的 Response、流式事件和 SDK 异常，建立可回放的调用结果分类。', ['Python', 'OpenAI', 'Responses API'], 'implementation', ['official-responses-api', 'example-model-adapter'], ['messages-tokens-context']),
    article('structured-output-model-boundaries', '结构化输出能约束什么，为什么仍不能证明业务事实', '从一个搜索候选拆开 Structured Outputs、JSON Schema、Pydantic 与认证上下文，说明格式正确为什么仍不能直接执行。', ['Structured Outputs', 'JSON Schema', 'Pydantic'], 'concept', ['official-structured-output', 'kb-runtime-contracts'], ['python-openai-responses-first-call']),
    article('agent-essence-autonomy-boundaries', 'AI Agent 的自主性从哪里来：模型提议、运行时准入与事实责任', '沿远程访问申请的反馈轨迹，区分固定工作流与 Agent，说明模型提议、运行时准入、审批、工具回执和完成验证各自负责什么。', ['Agent', 'Autonomy', 'Runtime'], 'concept', ['wl-01-agent-essence', 'kb-runtime-boundaries'], ['structured-output-model-boundaries']),
    article('python-agent-loop-from-scratch', '用 Python 写 Agent 循环：模型候选如何推动状态转移', '用不可变状态、类型化候选和完成验证器实现有限的 Python Agent Loop，并用两条轨迹验证自环、审批与终态。', ['Agent Loop', 'Python', 'State Machine'], 'implementation', ['wl-02-react-loop', 'example-agent-loop', 'kb-runtime-state'], ['agent-essence-autonomy-boundaries']),
    article('agent-fit-deterministic-workflow', '哪些任务不该使用 Agent', '用控制拓扑、证据变化和风险边界判断任务应停留在固定程序、工作流，还是升级到受限 Agent。', ['Agent', 'Workflow', 'Architecture'], 'concept', ['wl-01-agent-fit', 'appendix-pattern-selection'], ['python-agent-loop-from-scratch']),
  ]),
  ...stage('tools', [
    article('tool-calling-contracts', 'Tool Calling 的提议、校验与执行契约', '沿一次工具请求拆开目录、模型候选、Schema、可信授权、调用关闭、动作核对和未知副作用。', ['Tool Calling', 'Contract', 'Authorization'], 'implementation', ['wl-03-tool-calling', 'kb-tool-contracts'], ['agent-fit-deterministic-workflow']),
    article('parallel-tool-execution', '并行工具执行：如何保证身份、预算与结果收敛', '沿一次三路只读调查说明并行准入、调用身份、分维度预算、取消边界与部分结果归约。', ['Tool Calling', 'Concurrency', 'Budget'], 'implementation', ['wl-44-parallel-tools', 'kb-parallel-retrieval'], ['tool-calling-contracts']),
    article('mcp-foundations-boundaries', 'MCP 的系统位置与能力边界', '从 Host、Client、Server 和能力协商理解 MCP 与 Tool Calling、API、Skill 的关系。', ['MCP', 'Tool', 'Protocol'], 'concept', ['wl-04-mcp', 'kb-mcp-boundaries'], ['parallel-tool-execution']),
    article('mcp-protocol-lifecycle', 'MCP 从初始化到关闭的协议生命周期', '沿一次 search_policy 调用解释初始化、工具发现、消息关联、进度与超时、断线恢复和传输关闭。', ['MCP', 'JSON-RPC', 'Lifecycle'], 'implementation', ['wl-04-mcp', 'official-mcp-lifecycle'], ['mcp-foundations-boundaries']),
