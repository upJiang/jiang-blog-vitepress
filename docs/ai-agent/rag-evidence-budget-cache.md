...
Evidence Cache 的键至少绑定规范化查询、租户范围、知识 Release、Policy 和检索配置。命中后重新做 ACL 与新鲜度检查，候选过期或被撤回就失效。Singleflight 只合并并发计算，不改变每个调用的授权。

预算选择优先覆盖问题中的关键维度，再保留来源定位和冲突证据。缓存指标区分命中、重新鉴权、淘汰和降级，避免用命中率掩盖陈旧答案。
