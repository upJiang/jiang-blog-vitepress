---
title: 结构化输出是什么：从模型 JSON 到可信业务命令
description: 先区分 JSON、Schema 和结构化输出，再用 Pydantic 校验模型候选，并由服务端装配权限、版本与 Deadline。
category: ai-agent
part: 模型基础与系统地图
chapter: 4
tags:
...
lastUpdated: false
---
# 结构化输出是什么：从模型 JSON 到可信业务命令

## 结构化输出解决什么问题

结构化输出是模型按照预先约定的字段和类型返回数据。它适合意图识别、查询改写和工具参数这类需要交给程序继续处理的结果；最终写给人看的长答案，通常不需要先拆成一堆字段。

它位于模型生成与业务命令之间：模型先给出候选对象，Schema 和领域校验再决定这个对象能否进入检索、工具或状态更新。结构化输出解决的是数据交换的可检查性，不替代认证、权限和业务事实判断。
...
这里真正要解决的不是“怎样让模型吐 JSON”，而是从模型候选走到可执行命令时，哪一步由谁负责：
...
从模型输出到 `SearchCommand` 的受控链路位于 Agent 的结构化理解节点，输出会成为 SearchPlan 的语义输入。
...
这七步并非七个微服务。它们是七种责任，可以在同一个进程完成，但不能混成一次 `json.loads()` 后直接执行。

## JSON、JSON Schema 与结构化输出的职责区别
...
下面用 Pydantic 2 实现最小链路。实践不调用在线模型，而是把“模型原始 JSON”作为输入，这样可以稳定复现正常和失败分支。
...
把下面实现保存为 `structured_boundary.py`。它定义模型字段、检查字段关系，再把通过校验的候选与服务端上下文合成查询命令。
...
## Schema 进入队列或 Checkpoint 后就是内部协议

字段变更会影响历史响应、队列消息、缓存和 Checkpoint，不能只改 Pydantic 类然后上线。

| 变更 | 典型风险 | 处理方式 |
| --- | --- | --- |
| 新增必填字段 | 旧快照缺字段，恢复失败 | 保存 `schema_version`，先迁移旧对象 |
| 字段重命名 | 旧消费者仍读取旧键 | 消费者先兼容，新生产者后启用 |
| 删除枚举值 | 队列中的旧值无法反序列化 | 保留迁移器，等旧任务排空再删除 |
| 增加枚举值 | 旧消费者没有路由分支 | 先发布未知值兜底，再开始生成新值 |

例如把 `query` 改成 `normalized_query`，不只是命名优化。事件、缓存和评测快照都可能保存旧键。显式迁移比让 Pydantic 猜字段更容易审计，也能明确旧兼容逻辑何时可以删除。
...
1. 从下游消费者需要的字段开始，逐项标记来源、所有者和验证者。
2. 把身份、权限、金额、版本、Deadline 和状态转换移出模型 Schema。
3. 核对供应商支持的 Schema 子集，再限制额外字段、枚举、长度和嵌套规模。
4. 先检查响应信封，再做结构校验、跨字段校验和可信上下文装配。
5. 分开 refusal、incomplete、timeout、schema_error 和 domain_rejected；修复次数继续受同一个 Deadline 限制。
6. 为 Schema 保存版本，测试正常、缺字段、错类型、矛盾组合、越权字段和旧版本迁移。
7. 另用 Eval 检查语义，不把格式通过率当成业务准确率。
