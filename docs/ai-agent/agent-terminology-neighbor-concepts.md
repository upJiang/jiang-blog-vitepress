...
用“读者把 Message、Memory、RAG 和 Evidence 混为一谈”做自测时，入口先记录输入摘要、身份和执行版本。层级、状态所有者、输入输出和主文章链接由唯一组件写入，其他组件只能提交带修订号的候选。在“术语判断”里，读者可以从任务 ID 找到它经过的节点，不必靠最终文案猜过程。

针对“术语判断”，正常路径从准入开始，先检查范围和资源，再创建一次调用。调用结果带稳定 ID、来源和终止原因，结构合法但层级错误或把候选当事实分别记录。针对“层级、状态所有者、输入输出和主文章链接”，输出进入下一节点前还要确认版本和权限，空结果也要说明范围。

在“术语判断”的故障测试中，依赖调用前和结果已经产生但状态尚未提交时各切断一次。前者应没有外部调用，后者要留下可重放的未知状态。恢复使用同一幂等键，先查询外部回执，再决定补偿。

“术语判断”的验证命令检查输入、状态变化、调用顺序、输出摘要、失败证据和资源清理。回到负责该状态的组件。

“术语判断”的取舍需要写在结果里：扩大缓存、并行或自动修复可能缩短等待，也会增加状态、成本和失败组合；保持较小能力面则更容易做权限隔离和人工接管。

回放“术语判断”时改变输入顺序、重复提交、缩短 Deadline 或撤回权限，确认状态单调、范围不扩大、资源按时释放。指标用于定位变化，不替代事实核验。

## 术语判断 的落地方法

判断一个概念属于哪一层，可以先问三个问题：它是否改变模型输入，是否持有运行状态，是否拥有业务事实的写入权。Prompt 主要改变输入，Context 负责装配，Runtime 持有状态，Tool 执行外部能力，RAG 提供证据；如果一个对象同时承担多个职责，就需要拆成可验证的接口。

术语表不能替代设计。写下名称后，还要给出最小输入、输出、所有者和失败终态，再用一次调用轨迹检查它是否真的遵守定义。这样新成员读到“记忆”“知识库”或“Agent”时，能沿字段和事件找到实际边界。

## 术语判断 的边界案例

在“术语判断”中，入口收到的资料可能不完整，程序先保存缺口和当前版本，不能让模型用常识填入 层级、状态所有者、输入输出和主文章链接。“术语判断”的输入后来补齐时，新的执行沿同一个任务 ID 继续；已经结束的失败记录不被覆盖。

结构合法但层级错误时记录父条目、子条目和冲突版本，把候选当事实时记录缺失的来源与证据。没有匹配、解析失败、找到条目但缺主文章、定义仍待审校分别标记，避免用格式通过掩盖知识错误。

术语判断 的正常输出先经过范围、版本和权限检查，再进入交付或下一个节点。回到负责该状态的组件。“术语判断”的输出只满足结构而没有可验证来源时，状态保持为候选，前端应显示缺口而不是成功标记。

用重复提交、并发完成、取消和进程退出四个样本回归。回归术语判断 时，检查同一幂等键是否只产生一个有效终态，迟到事件是否被拒绝，临时资源是否释放，重新连接是否能读到同一份事件。

“术语判断”这组检查的结果应带命令、解释器或服务版本、输入摘要和退出码。术语判断 没有运行证据时，只能写“待验证”；离线替身通过也不能推导真实模型、网络服务或生产权限已经通过。

在“术语判断”的收尾检查中，把结构合法但层级错误或把候选当事实放在已经产生结果和还没有提交状态之间，记录调用是否发生、回执是否可查、当前版本是否变化。回到负责该状态的组件。在“术语判断”中，如果操作者只能看到一条失败文案，说明事件摘要仍不够，先补日志和状态字段，再讨论换模型或增加重试。
