...
`embedding_text` 需要说明是否包含标题、路径、表头、邻接摘要和显示格式。回答文本与检索文本可以不同，但要通过稳定版本字段关联，不能上线后凭记忆重建。

切换模型、维度或归一化方式会改变距离分布和索引兼容性。先建立精确基线与离线回放，再以候选 Release 激活，旧任务继续使用创建时的表示版本。
