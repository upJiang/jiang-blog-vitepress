...
发布后的对账要有明确窗口。切换新版本后，先观察创建成功率、队列年龄、Lease 超时、终态事件重复和 Evidence 支持率，再扩大分流范围。对账任务按 `turn_id`、`action_id` 和事件序号读取两套版本的记录，发现旧 Worker 写入新 Schema、事件缺帧或索引 Release 未激活时，只修复对应对象并保留原始错误证据。观察期内不要批量删除旧镜像、旧事件或回滚快照，确认没有活动 Owner 和未完成外部动作后再清理。这样回滚仍然有事实可查，问题也不会被一次全量重跑掩盖。
