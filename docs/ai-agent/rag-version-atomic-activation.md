...
先把验证对象分开，避免十几个用例堆成一段看不出各自在证明什么：

| 测试层 | 注入点 | 必须证明的结果 |
| --- | --- | --- |
| 状态机 | 投影仍为 pending、Release 已 active、旧 Release 已 retired | 不提前激活，重放幂等，终态不倒退 |
| 数据库事务 | Chunk 数量错误、两个候选并发竞争 | 失败完全回滚，最终只有一个 active |
| Worker 编排 | RAG、Wiki、Graph 不同顺序完成，旧任务迟到 | 只有全部 ready 才激活，迟到结果不能覆盖新版本 |
| 缓存与查询 | 预热候选、激活、回滚 | 普通请求只读活动版本，飞行任务不跨版本共享 |
| 进程恢复 | 提交前、事务中、提交后未发事件、最后投影 ready 后退出 | 分别重建、回滚、补发事件或幂等激活 |
...
候选 Release 在解析、Embedding、索引和质量门禁全部完成后才能激活。激活事务更新唯一活动指针并写 Outbox，查询开始时读取的版本在整个 Turn 内保持不变。

旧版本保留到活动任务和缓存都安全过渡。回滚只切活动指针，不删除原始证据；长任务完成后不能把旧快照写回当前知识。
