...
补偿任务本身也要幂等。RAG 失败后重新入库，先确认旧候选没有活动引用，再创建新的投影尝试；Wiki 已经生成过的卡片按 Source Content Hash 判断是否可以复用；Graph 已经写入的边按构建版本隔离，不能直接在活动图上覆盖。补偿失败时保留待清理记录，而不是在下一次发布中静默吞掉。
界面应分别显示“候选构建失败”“候选等待依赖”“旧 Release 仍在服务”和“候选已被更新版本取代”。四种状态的处理动作不同：失败需要修复输入或依赖，等待需要查看投影，旧 Release 需要决定是否继续服务，取代则不必重试。一个通用的红色失败标签会让操作人员反复点击已经不可能成功的任务。
