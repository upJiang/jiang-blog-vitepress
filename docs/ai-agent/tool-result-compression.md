...
工具结果压缩是把完整执行结果转换成有限模型视图的确定性处理。它位于工具执行和上下文装配之间：原始结果继续用于审计、分页和复查，模型视图只保留当前判断需要的字段、覆盖范围、截断状态和来源指针。
...
空列表表示工具正常完成且没有匹配，超时表示系统不知道是否存在结果。把二者合并会让 Agent 错误回答“没有资料”，也可能停止本应有限重试的研究。ToolResult 使用成功、empty、error、cancelled 等联合类型，错误包含稳定 code、retryable 和 source ID；模型视图可以缩短错误详情，但不能抹掉状态。
