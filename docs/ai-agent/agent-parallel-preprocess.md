...
并行预处理是 Agent Runtime 在 Planner 之前建立可信输入的阶段。它位于准入/快照之后、计划和工具执行之前，负责把安全扫描、上下文预算、授权记忆和快速匹配结果汇成一份带状态的输入；它不是让所有函数同时启动的性能开关。

只有互不依赖、共享输入只读、资源预算可计算的分支才适合并行。合并节点仍由程序决定哪些失败可以降级，哪些失败应阻断后续计划。

一次请求进入 Runtime 后，安全扫描、历史裁剪、用户记忆读取和错误码快速匹配往往互不依赖。串行执行会让每个步骤等待前一步；无边界地**并行**又会争抢连接、Token 和 CPU。有限预处理阶段要保持共享输入只读，定义结果合并方式，并明确单个分支失败是否影响主流程。
