...
视觉链路的排障先问“最后一个可信证据在哪里”。入口没有建立会话，属于输入或授权层；图像没有采集成功，属于环境适配器；图片进入了错误会话，属于引用或状态层；压缩后文字不可读，属于表示层；动作参数越界，属于策略层；浏览器没有回执，属于执行器或渠道层；业务状态没有变化，属于领域验证层。

相同的错误文案不能覆盖这些阶段。至少要保存请求 ID、会话 ID、图像 ID、页面版本、动作尝试 ID、错误类、是否可重试、是否产生副作用和资源清理结果。日志只记录稳定标识和错误类别，原图、用户目标、Cookie 和完整 Prompt 留在受控存储，避免排障日志变成新的泄露入口。

故障恢复遵循拥有状态的边界。采集失败可以重新截图，压缩失败可以回到原图，旧页面缓存可以先查询权威版本，策略拒绝不能靠重试绕过，执行结果未知必须先查回执，业务验证失败可以停止交付并保留差异。每一种重试都消耗同一份 Deadline、动作预算和调用配额。
