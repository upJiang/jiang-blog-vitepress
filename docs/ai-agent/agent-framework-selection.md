...
Agent 控制模式是一组决定 Runtime 下一步怎样推进的状态转换规则，用于给不同任务选择有限路由、观察循环、先规划后执行或生成后复查。Router 在有限路径中选一条，ReAct 根据工具观察逐步决定，Planner 先生成有限计划再执行，Reflection 对候选结果做受限复查。它们位于模型候选和程序执行之间；框架只是实现这些状态变化、恢复和观测的工具。

控制模式的用途是给一类任务选择合适的执行形态，避免简单问题承担不必要的循环和状态。它们属于 Agent Runtime 的决策层，框架选型必须建立在模式定义和适用边界之上。

[有限 Agent 循环](/docs/ai-agent/python-agent-loop-from-scratch) 已经跑通“模型提出动作、程序执行工具、观察回到模型”的最小路径。任务从一次检索扩展为多种问题后，Runtime 还要选择与任务复杂度匹配的执行方式。
...
先写需求，再为每个候选打 0 至 2 分：0 表示关键能力缺失，1 表示需要适配，2 表示直接满足。分数必须附验证证据。
