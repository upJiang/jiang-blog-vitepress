...
根据任务不确定性、依赖、质量、延迟、成本和风险选择最小可行模式。

**Pattern Selection**、**Decision**、**Agent** 分别指向同一条执行链上的不同对象：任务目标、分支可预测性、外部动作、依赖关系、质量阈值、Deadline、预算和失败代价进入系统，decision_id、candidate_pattern、required_state、tool_scope、cost_envelope、stop_rule、

fallback 和 rejection_reason 保存处理中间状态，最终得到模式选择、拒绝其他模式的具体原因、状态需求、停止条件、验证方式和降级方案。
