...
## 重新设计时先问资源由谁承诺
准入、容量 Lease 与执行 Lease 的边界决定了故障责任。入口只承诺可以进入队列，Worker 取得执行权后才占用运行资源，恢复器只能依据过期时间和 fencing token 接管。把三种状态压成一个布尔锁，重试、排队和失联就会互相覆盖。

验收时记录一次被拒绝的准入、一次租约续期失败和一次旧 Worker 提交被拒绝的 Trace。测试替身只能验证状态机顺序，真实存储仍需验证原子计数、时钟漂移和网络分区。
