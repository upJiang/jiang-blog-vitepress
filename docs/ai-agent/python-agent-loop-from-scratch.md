...
practice:
  type: implementation
  result: 得到可测试的有限循环核心，并覆盖正常、空结果、重复、工具错误和步数耗尽路径
  verify:
    - 正常问题产生有证据回答
    - 重复调用和达到上限时确定停止
...
Agent 循环是 Runtime 反复执行“读取当前状态、请求模型提出下一步、校验并执行工具、追加观察、检查停止条件”的控制结构。它位于模型网关和业务终态之间：模型负责提出候选动作，程序负责状态、权限、预算和结束条件。只有一轮模型调用的应用不需要这层循环。

这里采用有限、只读的循环核心，完整路径是 `ToolCall -> ToolResult -> 下一次模型请求 -> 最终回答`。未知工具、空结果、重复调用和步数耗尽都必须转换成可观察终态。
...
为了让循环可测试，模型策略与工具执行都通过协议注入。输入是当前消息快照，输出分别是 `ModelDecision` 和 `ToolOutcome`。下面按文件边界组织代码：
...
│   ├── schemas.py       # ToolDecision、FinalDecision、ModelDecision
│   ├── tools.py         # DecisionModel、ToolExecutor、ToolOutcome
│   └── agent_loop.py    # run_agent 与终态
└── tests/test_agent_loop.py
```

这不是一个已经发布的仓库；它是把本文代码复制到对应文件后即可运行的最小练习目录。`app.*` 只表示这些代码块之间的导入边界，不代表读者需要安装名为 `app` 的第三方包。
...
def test_empty_result_cannot_become_a_confident_answer() -> None:
    model = ScriptedModel([
        ToolDecision("tool_call", "call-empty", "search_notes", {"query": "不存在"}),
        FinalDecision("final_answer", "资料说明这项制度一定存在。"),
    ])
    result = run_agent("查制度", model, SearchNotesExecutor())
    assert result.status is RunStatus.NO_EVIDENCE

def test_unknown_tool_stops_before_a_second_model_step() -> None:
    model = ScriptedModel([
        ToolDecision("tool_call", "call-unknown", "delete_notes", {}),
    ])
    result = run_agent("删除资料", model, SearchNotesExecutor())
    assert result.status is RunStatus.FAILED

def test_duplicate_tool_call_is_stopped() -> None:
    decision = ToolDecision("tool_call", "call-1", "search_notes", {"query": "切流验证"})
    model = ScriptedModel([decision, decision])
    result = run_agent("怎样验证切流？", model, SearchNotesExecutor())
    assert result.status is RunStatus.FAILED
    assert "重复" in result.answer

def test_different_calls_still_hit_the_step_budget() -> None:
    model = ScriptedModel([
        ToolDecision("tool_call", "call-1", "search_notes", {"query": "一个问题"}),
        ToolDecision("tool_call", "call-2", "search_notes", {"query": "另一个问题"}),
    ])
    result = run_agent("需要多次查询", model, SearchNotesExecutor(), max_steps=2)
    assert result.status is RunStatus.FAILED
    assert result.steps == 2
```
...
这五个测试分别覆盖正常完成、空结果、未知工具、重复调用和步数耗尽。它们证明“有限”由程序控制，而不是 Prompt 里的一句建议；真实模型接入后仍需另测协议转换和供应商事件。
...
模型可能重复相同调用、产生无效参数，或始终认为证据不足。没有上限的 `while True` 把停止权交给不确定输出，无法限制费用和时间。程序至少要固定最大步数、整轮 Deadline、重复动作、取消信号和不可重试错误。

`max_steps` 取决于任务和工具链，不是通用常量。单次知识检索通常两步就能完成，多子问题研究可能需要更多。可以用 Eval 记录成功样本的步骤分布，再为长尾设置上限；达到上限必须暴露为明确终态，不能静默返回半成品。

只读知识 Agent 承诺基于当前可见资料回答。工具成功但无结果表示证据缺口，不等于事实不存在，也不授权模型用训练记忆补齐。可以允许一次查询改写，仍为空时应解释资料不足。`empty` 表示检索完成但当前 Scope 和 Release 下没有结果，`failed` 表示查询没有可靠完成，两者不能共用一个终态。

本文的 JSON 指纹只能发现结构完全相同的重复，`"切流验证"` 与 `"验证切流"` 仍是不同指纹。更完整的 Runtime 可以同时比较动作指纹和新增信息：保守规范化查询后，再检查本轮是否增加 Evidence ID 或覆盖目标。字符串不同却没有增加证据时，继续循环只会增加成本；向量相似度也不能直接合并所有查询，否则会误伤版本号和专有名词。

### 错误分类与最终回答验证

参数格式错误可以给模型一次修正机会，空结果可以改写查询；越权、取消和未知工具不应通过换种表达绕过。超时只有在整轮仍有预算且操作幂等时才适合有限重试。调试记录应包含每步决策类型、工具指纹、剩余时间和停止原因，而不是只保存最终文本。

工具返回正确证据，也不代表模型没有漏项、错引或添加无依据结论。这个实现只检查空结果边界；后续的 Claim、Evidence 与验证器会把回答拆成可核对事实，在提交终态前检查引用、权限和隐私。

### 测试替身与在线评测

Fake 不能评估模型质量，但适合验证确定性控制，包括调用次数、消息配对、重复检测和终态。在线模型通过 Eval 覆盖自然语言理解和工具选择。两类测试负责不同风险，不能互相替代。
