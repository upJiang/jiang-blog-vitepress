...
lastUpdated: false
---
# Celery Worker、ACK 与 Lease：任务为什么会重复执行
...
Broker 可见性或 ACK 机制决定消息何时重投，业务 Lease 决定哪个 Worker 当前有权推进任务状态和提交结果。两者可能不同步：消息已重投时旧 Worker仍在运行，只有 fencing token 能阻止双写；业务 Lease 过期也不一定自动让 Broker 立刻投递。系统需要同时配置并观察，不能把 Broker 超时当业务所有权。恢复器按数据库 Lease 接管，ACK 再按执行结果处理。
