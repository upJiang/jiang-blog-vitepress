...
它们的用途是把一次请求的规则、历史、证据和问题装配成可检查的模型输入，方便做裁剪、注入隔离和快照测试。Prompt 负责生成结构，Message 负责承载结构；权限和证据可见性仍由更上游的程序决定。
...
下面沿用一个只读知识问答场景：用户先问“远程办公需要什么设备条件”，又追问“那访问权限怎么申请？”系统检索到两段可见资料，然后装配模型输入。
...
示例只接受文本 HumanMessage 与 AIMessage。这个限制用于固定输入边界，不代表 LangChain 只能处理文本。
...
示例只设八条上限，真实系统应按 Token 预算而非消息条数裁剪。建议顺序：
