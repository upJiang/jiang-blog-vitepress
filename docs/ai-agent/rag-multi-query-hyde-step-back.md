...
## Multi-Query、HyDE 与 Step-back 分别是什么

Multi-Query 是一种为同一问题生成几条不同表达的扩展策略，HyDE 是一种先生成假设性答案文档再用它检索的方法，Step-back 则把具体问题提升为更一般的背景问题。三者都位于查询理解和 Retriever 之间，用于缩小用户用词与知识库原文之间的表达差异；它们增加的是召回机会，不会修复错误权限或缺失知识。
