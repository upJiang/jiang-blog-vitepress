...
遇到缺口时只做定向补搜，补搜没有带来新覆盖就停。“研究停止”的引用被撤回或版本互相冲突时，系统保留缺口与冲突，不用更长的摘要把不确定性藏起来。删除无法定位的 Claim，明确留下缺口。

“研究停止”先用固定来源快照验证覆盖度、引用绑定、重复来源和停止原因，再把一个过期链接和一条相反材料插入样本。“研究停止”的输出应说明哪些 Claim 被保留，哪些被删除，以及研究在哪一轮停止。

研究结果还要记录预算和取消信号。“研究停止”收到取消后不再创建新的检索分支，已经返回的 Evidence 可以用于解释，但不能绕过最终权限复核。“研究停止”的离线测试只证明控制逻辑，开放网络的内容仍需在目标环境核对。

“研究停止”的回归报告比较覆盖增长、有效引用数、空结果和未确认状态，不把一次回答长度或模型偏好当成研究质量。

## 研究停止的边界案例

在“研究停止”中，入口收到的资料可能不完整，程序先保存缺口和当前版本，不能让模型用常识填入 Claim、Evidence、来源版本、覆盖维度和停止原因。“研究停止”的输入后来补齐时，新的执行沿同一个任务 ID 继续；已经结束的失败记录不被覆盖。

链接撤回要更新来源版本并让相关 Claim 失效，来源冲突要保留双方证据，补搜没有新增覆盖则记录已尝试的查询和预算。未发起检索、检索失败、没有新增证据、停止原因尚未确认分别处理，不能把空结果解释成充分覆盖。

“研究停止”的正常输出先经过范围、版本和权限检查，再进入交付或下一个节点。删除无法定位的 Claim，明确留下缺口。“研究停止”的输出只满足结构而没有可验证来源时，状态保持为候选，前端应显示缺口而不是成功标记。

用重复提交、并发完成、取消和进程退出四个样本回归。回归“研究停止”时，检查同一幂等键是否只产生一个有效终态，迟到事件是否被拒绝，临时资源是否释放，重新连接是否能读到同一份事件。

这组检查的结果应带命令、解释器或服务版本、输入摘要和退出码。“研究停止”没有运行证据时，只能写“待验证”；离线替身通过也不能推导真实模型、网络服务或生产权限已经通过。
