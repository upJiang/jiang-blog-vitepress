...
模型返回一段语法正确的中文，并不等于它可以直接展示。用户问“退款期限是多少”，答案可能引用了错误版本的政策，或用公开 FAQ 支撑企业客户专属条款。工程上必须检查每个 Claim 是否有证据、证据是否对当前用户可见，以及问题能否在预算内修复。

下面会实现一个离线验证器：输入 Claims、Evidence 和引用关系，输出 `approved`、`repairable` 或 `refused`。这段代码不调用模型，每个判断都能被测试和复现。
