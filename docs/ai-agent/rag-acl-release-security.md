...
## Release 与 ACL 是同一条证据链
查询开始时固定知识 Release，随后在候选、缓存、重排、引用和答案验证各层携带权限范围。缓存命中也要重新检查当前 ACL，撤回版本后旧候选不能继续支撑新回答。

安全测试至少包含同文档跨租户、撤回后缓存、长任务跨发布和图谱扩展越界。任何一层缺少版本或范围字段，都应阻止发布而不是补默认值。
