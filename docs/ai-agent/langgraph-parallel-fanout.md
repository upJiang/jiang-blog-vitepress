...
错误码适合精确或全文检索，同义问题适合向量检索，表格字段适合结构化查询。把三条通道串行执行会累加延迟；把它们直接 `asyncio.gather` 在一个节点里，又会隐藏每个分支的状态和错误。本篇用 LangGraph `Send` 把研究计划动态拆成分支，再由融合节点统一处理结果。

本篇的完成结果不是“把三个函数同时跑起来”，而是得到一条可解释的并行链：父状态在**扇出**前固定问题、权限范围和预算；每个分支只携带自己的通道参数；分支成功、无结果和失败都转换成结构化信封；融合节点按稳定规则去重、排序并决定是否继续。这个顺序决定了并行不会绕过权限或把一个异常伪装成“没有资料”。
