...
策略对象不能只有一段 Prompt。它至少要固定模型、工具目录、参数约束、预算、验证器、发布范围和生效时间。运行时保存 `policy_id` 与版本摘要，才能解释某个动作为何被允许或拒绝。

策略评测应包含安全拒绝、合法读操作、超预算和版本并存。Shadow 只观察不产生副作用，Canary 受租户和流量上限约束，回滚时要处理缓存与长任务的旧快照。
