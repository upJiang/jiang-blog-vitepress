...
在接入框架前，可以先用纸笔演练“比较两份规范”的请求。准备两段短文，故意让第二段缺少更新时间，然后逐格填写下面的运行记录。这个练习的重点不是让模型写出漂亮答案，而是看每一步是否有可交给下一步的明确产物。

| 阶段 | 本阶段输入 | 应留下的产物 |
| --- | --- | --- |
| 理解 | 用户原话、身份、指定范围 | 目标、对象、限制和待澄清项 |
| 计划 | 已确认目标 | 可执行步骤与停止条件 |
| 工具 | 工具名、已校验参数 | 带来源和时间的结构化结果 |
| 观察 | 工具结果、剩余预算 | 继续、改计划、询问或停止 |
| 生成 | 已授权证据 | 带证据身份的候选回答 |
| 验证 | 候选回答、证据集合 | 通过原因或具体错误 |

当第二份资料缺少更新时间时，观察阶段应保留这个缺口。生成阶段可以比较已有正文，但结论要说明无法判断哪份更新，不能把“文件排列在后面”推断成“版本更高”。这正是观察与验证存在的价值。

实现时也可以用同一张表检查日志。如果工具结果里有正确资料，而答案仍然写错，问题在生成或验证；如果工具根本没找到资料，就先检查理解、查询改写和检索。把错误定位到阶段，比继续改一段全局 Prompt 更容易形成可重复修复。

### 一个可迁移的练习

换成你熟悉的任务，例如“整理一次构建失败原因”。写出七个阶段，限制最多两个工具和三次循环，再回答三个问题：什么时候必须询问用户？什么结果算证据不足？哪个条件触发终止？能回答清楚，才算真正掌握了 Agent 生命周期。
