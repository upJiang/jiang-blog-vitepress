...
Research 的离线评测关注质量变化，运行时测试关注控制和恢复，两类证据都齐全才可交付。研究综合判断覆盖度与冲突只有两类证据都存在，才能同时回答“结果是否合格”和“系统是否安全完成”。

## 并发、预算和交付策略

研究任务要冻结问题维度、来源版本和综合结果 Schema。维度变化时，旧报告仍按旧覆盖定义读取，新报告必须明确新增或删除了哪些维度。
...
出现覆盖率下降时，先用 `request_id` 找研究问题、来源清单、维度缺口、冲突证据和合并裁决，再判断要补搜、撤回来源还是保持不确定。摘要文本不是定位依据。
...
研究综合判断覆盖度与冲突的监控阈值要对应可执行动作。Research 的分支、依赖、上下文和预算接近上限时，应限流或缩小候选；告警本身不会停止正在运行的任务。
...
摘要压缩表达，覆盖度判断是否每个问题维度都有独立来源。研究控制器决定补搜和停止，来源记录保留原文与版本，综合器只能在证据范围内提出结论。
...
把“一组支持和反驳材料需要合并成结论”做成可复现的研究样本，入口先登记问题边界、时间范围和允许的来源。Coverage、Claim、证据独立性和冲突记录必须能回到原始材料、版本和访问范围，不能由模型在摘要里自行补全。

正常路径先生成有限的 SearchPlan，再按维度取回候选。每条候选保存来源 ID、发布时间、检索通道和独立性判断，摘要缺来源、同一文档重复计数或版本漂移分别写入 Evidence 状态。综合时只有通过范围和新鲜度检查的证据才能支持 Claim。

遇到缺口时只做定向补搜，补搜没有带来新覆盖就停。“一组支持和反驳材料需要合并成结论”的引用被撤回或版本互相冲突时，系统保留缺口与冲突，不用更长的摘要把不确定性藏起来。结论范围跟随最弱证据。

“一组支持和反驳材料需要合并成结论”先用固定来源快照验证覆盖度、引用绑定、重复来源和停止原因，再把一个过期链接和一条相反材料插入样本。“一组支持和反驳材料需要合并成结论”的输出应说明哪些 Claim 被保留，哪些被删除，以及研究在哪一轮停止。

研究结果还要记录预算和取消信号。“一组支持和反驳材料需要合并成结论”收到取消后不再创建新的检索分支，已经返回的 Evidence 可以用于解释，但不能绕过最终权限复核。“一组支持和反驳材料需要合并成结论”的离线测试只证明控制逻辑，开放网络的内容仍需在目标环境核对。

“一组支持和反驳材料需要合并成结论”的回归报告比较覆盖增长、有效引用数、空结果和未确认状态，不把一次回答长度或模型偏好当成研究质量。


## 一组支持和反驳材料需要合并成结论的边界案例

在“一组支持和反驳材料需要合并成结论”中，入口收到的资料可能不完整，程序先保存缺口和当前版本，不能让模型用常识填入 Coverage、Claim、证据独立性和冲突记录。“一组支持和反驳材料需要合并成结论”的输入后来补齐时，新的执行沿同一个任务 ID 继续；已经结束的失败记录不被覆盖。

摘要缺来源要退回引用绑定，同一文档重复计数要按稳定来源 ID 去重，版本漂移要保留抓取时间和活动版本。没有检索、检索失败、检索完成但没有新增覆盖、覆盖仍未裁决分别记录，不能用一段摘要掩盖缺口。

“一组支持和反驳材料需要合并成结论”的正常输出先经过范围、版本和权限检查，再进入交付或下一个节点。结论范围跟随最弱证据。“一组支持和反驳材料需要合并成结论”的输出只满足结构而没有可验证来源时，状态保持为候选，前端应显示缺口而不是成功标记。

用重复提交、并发完成、取消和进程退出四个样本回归。回归“一组支持和反驳材料需要合并成结论”时，检查同一幂等键是否只产生一个有效终态，迟到事件是否被拒绝，临时资源是否释放，重新连接是否能读到同一份事件。

“一组支持和反驳材料需要合并成结论”这组检查的结果应带命令、解释器或服务版本、输入摘要和退出码。“一组支持和反驳材料需要合并成结论”没有运行证据时，只能写“待验证”；离线替身通过也不能推导真实模型、网络服务或生产权限已经通过。

research-synthesis-coverage的修改要优先改变责任边界和可观察证据，不能靠增加一个关键词特判来让样本通过。
