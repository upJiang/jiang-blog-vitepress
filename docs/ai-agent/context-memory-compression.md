...
短期记忆是一轮任务当前可用的上下文，长期记忆是经过授权后可跨会话读取的事实，滚动摘要则是对不断增长的会话历史做增量压缩。三者位于原始对话和下一轮上下文装配之间，用于按不同生命周期保存信息，而不是把所有历史统一塞进向量库。
...
为每种候选信息填写：所属 Scope、允许保留多久、是否敏感、谁确认、来源是什么、冲突键是什么、撤回怎样传播、装配优先级和验证方式。当前案例得到以下结果：
