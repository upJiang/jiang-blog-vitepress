...
Checkpoint 保存的是可恢复状态，不是永久通行证。进程崩溃后，Runtime 重新读取当前权限、Policy、Release 和剩余预算，确认尚未提交的动作可以重试，再从检查点推进。

取消与超时都要记录已提交副作用和未开始动作。外部调用没有幂等回执时，终态应保留 `unknown` 或转人工核对，不用重试掩盖不确定性。
