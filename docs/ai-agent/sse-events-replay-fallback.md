...
lastUpdated: false
---
# SSE 事件序号、断线重放与轮询降级：流式 Agent 怎样不丢状态

**SSE**（Server-Sent Events）让浏览器通过一个长连接接收文本事件，适合展示 Agent 的检索进度、引用和最终答案。但网络切换、代理超时或手机休眠都会让连接断开。只在内存里 `yield` 事件，重连后客户端无法知道自己漏了哪一段。
