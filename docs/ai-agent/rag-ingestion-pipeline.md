...
RAG 数据导入要解决三个工程问题：**怎样把不可信文件变成可追溯 Chunk，怎样在任一阶段失败后从确定位置恢复，怎样保证半成品永远不进入在线查询。** 本篇先建立整条状态链，后续文章再分别深入格式解析、切片、Embedding、向量库和索引。

开始前只需要知道文件、数据库记录和异步任务的基本区别。这里的公开例子使用匿名对象，不对应任何私有表结构。最终产物是一份导入状态表、一段可运行的状态机和一组恢复测试。
