...
Deep Research 的离线评测关注质量变化，运行时测试关注控制和恢复，两类证据都齐全才可交付。Deep Research 只有两类证据都存在，才能同时回答“结果是否合格”和“系统是否安全完成”。

## 并发、预算和交付策略

Deep Research 要冻结研究问题、SearchPlan、Coverage 维度、来源版本和引用 Schema。查询策略升级后，旧研究仍按当时的计划回放，新的来源不能悄悄改变旧结论。

Deep Research 的容量主要受分支数量、来源抓取、共享工作区冲突、综合调用和截止时间影响，并发可能缩短单次等待，也会占用连接、配额、内存和下游吞吐，必须沿拆分问题、分派窄任务、并行取证、处理冲突、汇合结果逐层核算。

Deep Research 的候选版本在固定样本上比较正确性、错误类型、延迟和资源消耗，查询不断改写却无新增证据等安全红线回归时直接停止发布，不能用平均质量抵消。
...
Deep Research 的数据按证据用途分级保留，聚合字段可以留得更久，父目标和大体积带来源的子结果设置短期限、访问控制与删除通道。

Deep Research 先在单进程实现研究问题、超时、错误和测试，只有恢复与容量证据提出要求时，再增加队列、Lease、分布式缓存或工作流引擎。

Deep Research 的监控阈值要对应可执行动作。Deep Research 的分支、依赖、上下文和预算接近上限时，应限流或缩小候选；告警本身不会停止正在运行的任务。

回滚要覆盖代码之外的状态。Deep Research 若改变 Schema、缓存键或持久化记录，旧版本是否还能读取新写入数据必须提前验证；无法双向兼容时采用候选版本隔离。

## 何时退回单 Agent

采用 Deep Research 前先画出父目标到带来源的子结果的最小控制图；固定函数已经能完成任务时，新增模型决策和跨进程研究问题只会扩大测试与运维范围。

普通搜索返回一次候选列表，Deep Research 还要维护问题分解、覆盖度和停止条件。控制器决定下一次查询，来源仓库保存原始证据，验证器确认子结果能否进入综合。

RAG 可以与 Deep Research 组合，但外部知识仍需授权，工具调用仍需校验，模型产生的带来源的子结果仍停留在候选状态。

Deep Research 适合价值足够、查询不断改写却无新增证据可观察且预算可接受的任务，高风险、不可逆的决定需要确定性规则或人工批准。

执行期间修改子问题可以检验并发设计，旧动作若仍能覆盖研究问题，说明版本或 Lease 有缺口，增加 Prompt 规则无法修复这类竞态。

Deep Research 增加了研究问题、依赖调用、恢复责任和故障面，设计记录既要说明获得的能力，也要列出新增的退出、降级与回滚路径。
