---
title: LLM、工作流、RAG 与 Agent：四种系统怎样分工
...
- **Agent**：运行时保存状态，模型提出候选动作，程序决定是否执行。

:::

这四个词不在同一层。**LLM** 属于模型层，**工作流** 属于应用编排层，**RAG** 横跨检索和生成，**Agent** 位于更上层的控制运行时。一个系统可以同时包含它们，但每个词承担的责任不同。

## 先用同一个问题看四条执行路径

固定一个问题：“我在家访问系统时被拒绝了，应该怎么办？”先不比较术语，沿着一次请求看五个字段：输入、状态、下一步的决定者、输出和失败终态。

| 路径 | 输入 | 状态保存 | 谁决定下一步 | 输出 | 失败怎样停止 |
| --- | --- | --- | --- | --- | --- |
| **LLM** | 问题和本次提供的资料 | 当前消息 | 模型继续生成 Token | 文本或结构 | 应用检查空响应、格式和业务条件 |
| **工作流** | 问题、身份和固定配置 | 程序状态机 | 代码条件和节点顺序 | 固定格式的处理结果 | 节点返回 rejected 或 failed |
| **RAG** | 问题、范围和知识版本 | 查询、候选和证据 | 检索策略与程序路由 | 带来源的候选上下文和回答 | 当前范围无证据时拒答或降级 |
| **Agent** | 问题、范围、预算和工具契约 | 消息、动作、观察和步数 | 模型提出候选，程序审核执行 | 终态答案、证据和执行事件 | 超时、越权、重复或达到上限时终止 |

同一个问题因此会得到不同的执行轨迹。LLM 可以直接组织说明；工作流按固定顺序读取规则；RAG 先找当前资料；Agent 查到第一批结果后，再决定是否需要第二个工具。页面看起来都像聊天框，控制权却不在同一个位置。
...
LLM 是 Large Language Model，大语言模型。应用把 System、User 等消息编码成 Token，模型根据训练阶段得到的参数和当前上下文，为下一个 Token 计算候选分布，再继续生成，直到遇到停止标记或输出上限。它位于模型层，负责处理语言的不确定性，不能直接读取数据库或改变业务状态。

::: info 为什么这四条都属于 LLM 的职责边界

它们共同回答一个问题：哪些事情由模型负责生成候选，哪些事情必须由应用、检索系统或业务程序确认。参数、上下文、生成方式和输出形状都影响模型能给出什么，但都不能替代事实查询、长期存储、权限检查和业务验证。
...
模型参数保存训练阶段学到的统计规律，适合生成语言候选，却不是可以按主键精确查询的事实表。当前账号状态、最新发布版本和私有文档不在参数里，模型就没有直接查询它们的通道。

### 上下文为什么不是长期记忆

上下文只属于本次调用，通常由系统规则、用户问题、历史摘要、检索证据和工具结果组成。调用结束后，除非应用显式持久化并通过权限和版本检查，内容不会自动进入下一次请求。

### 生成为什么不是数据库查询

生成是在候选 Token 中继续选择。它可以复述上下文，也可能产生多个语言上合理的续写；数据库查询则由条件、索引和事务规则决定结果集合。需要精确事实时，应用应先检索，再让模型处理已授权的证据。

### 结构化输出为什么不等于真实性证明

JSON Schema 或 Pydantic 可以约束字段、类型和枚举，但不能证明字段内容真实。Schema 校验通过后，程序仍要补充用户身份、Scope、Knowledge Release、PolicyVersion 和 Deadline，并再次检查证据与业务状态。
