...
这篇文章不会只给四个定义。我们会用同一个匿名场景逐步运行四种实现，观察它们的输入、内部状态、处理过程、输出和失败方式。读完后，你应该能为一个需求画出执行路径，并解释为什么某些需求只需要普通程序。
...
本文不会假设你已经了解 Token、向量数据库或 LangGraph。它们会先以可观察的形式出现，再在后续文章继续展开。
...
- “RAG 能消除幻觉”：RAG提供候选证据，仍需要引用和答案验证。
