...
## Message、Token 与 Context 分别是什么

消息是应用按角色组织模型输入的结构单元；Token 是模型分词器处理文本后得到的计量单元；上下文窗口是一次模型调用能够容纳的输入与输出总范围。它们位于应用装配和模型推理之间：应用决定发送哪些消息，模型按 Token 接收这些内容，窗口上限再约束能保留多少历史、证据和输出。

这三个概念一起用于设计请求预算、定位超限和截断问题，也让你能解释一次模型调用为什么提前停止。它们的依赖顺序是“消息对象 -> Token 计量 -> 窗口预算”，不能当成三个孤立术语。
