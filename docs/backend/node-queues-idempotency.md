---
title: "Node 队列、幂等与重试"
description: "从一次重复提交开始，理解任务记录、幂等键、至少一次投递、租约与有限重试。"
category: backend
tags: ["Node.js", "Queue"]
updated: 2026-08-05
order: 40
...
常见队列提供的是至少一次投递：消息最终会再次出现，因此消费者要接受重复。所谓“恰好一次”不能只靠 Broker 的 ACK；数据库提交、对象存储写入和外部请求都有各自的不确定窗口。
...
并非所有系统都已经使用 Transactional Outbox。若当前采用“提交后直接派发”，就要明确记录派发失败，并由扫描任务补发，不能把它描述成已经具备原子投递。两种方案的差异在于失败窗口是否有持久的待发布事实。
