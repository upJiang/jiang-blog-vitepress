...
三套 API 各开 30 个连接、每套 4 个副本，理论上会占 360 个连接；MySQL 上限只有 200。单个服务的池配置看似保守，叠加后仍会耗尽数据库。

## 一个连接占用服务器资源

一个连接占用服务器资源不能只靠术语记忆。先确定输入来自谁、状态由谁拥有、一次操作改变了哪些记录，再看输出如何被下一层使用。**状态所有者一旦含糊，重试和故障恢复就会出现重复或丢失。**

```mermaid
flowchart LR
  A[输入与上下文] --> B[一个连接占用服务器资源]
  B --> C[状态检查]
  C -->|满足约束| D[提交结果]
  C -->|不满足| E[稳定错误]
  D --> F[日志 / 指标 / 审计]
...
池大小来自总预算要放回请求时间线：开始时读到什么，中间获得什么锁、连接或租约，成功时提交什么，失败时又能否回到原状态。这样才能判断超时后是安全重试、查询原结果，还是进入人工对账。

下面的片段抓住 MySQL 中最容易出错的一条执行路径。先观察输入条件和状态标识，再看副作用在什么位置发生。

```sql
总连接预算 = API 副本数 × 每副本池上限
              + Worker 副本数 × 每副本池上限
              + 迁移、管理和恢复预留
```
...
排队、超时和连接泄漏最终要落实到可观察证据。MySQL的配置、日志或执行结果需要携带稳定标识，例如 requestId、资源版本、任务 ID 或制品摘要，避免只凭“看起来正常”判断系统。
...
### 排队、超时和连接泄漏与前一层的责任怎样交接？

交接内容写入契约：输入带身份、范围和版本，输出带结果、状态码和可追踪 ID。排队、超时和连接泄漏只处理自己拥有的状态。
