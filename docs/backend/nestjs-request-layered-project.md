...
最小发布接口跑通后，再增加一个后台任务入口，复用同一个 `PublishArticle` 应用用例。后台任务没有 HTTP 状态码，也不认识装饰器；它只构造命令、调用用例并处理业务结果。如果为了复用还要伪造 `Request` 或直接调用 Controller，说明 HTTP 细节仍然泄漏到了应用层。

| 层 | 输入 | 输出 | 不应承担 |
| --- | --- | --- | --- |
| Controller | HTTP 参数、认证上下文 | HTTP 响应 | 事务顺序、ORM 查询 |
| 应用服务 | 业务命令 | 用例结果 | Express/NestJS 响应对象 |
| 领域规则 | 明确业务状态 | 允许或拒绝 | 网络和数据库连接 |
| Repository | 查询/保存意图 | 领域对象或投影 | 决定业务流程 |
