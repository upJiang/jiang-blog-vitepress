...
直接给五百万行表增加 `NOT NULL` 列并要求旧应用立刻写入，会把发布和数据回填绑在一次操作里。安全演进通常分为扩展、回填、切换、收缩。

## 迁移是有顺序的数据变更

迁移是有顺序的数据变更不能只靠术语记忆。先确定输入来自谁、状态由谁拥有、一次操作改变了哪些记录，再看输出如何被下一层使用。**状态所有者一旦含糊，重试和故障恢复就会出现重复或丢失。**

```mermaid
flowchart LR
  A[输入与上下文] --> B[迁移是有顺序的数据变更]
  B --> C[状态检查]
  C -->|满足约束| D[提交结果]
  C -->|不满足| E[稳定错误]
  D --> F[日志 / 指标 / 审计]
...
## expand/contract 保持版本兼容

expand/contract 保持版本兼容要放回请求时间线：开始时读到什么，中间获得什么锁、连接或租约，成功时提交什么，失败时又能否回到原状态。这样才能判断超时后是安全重试、查询原结果，还是进入人工对账。

下面的片段抓住 MySQL 中最容易出错的一条执行路径。先观察输入条件和状态标识，再看副作用在什么位置发生。

```sql
ALTER TABLE projects ADD COLUMN owner_id BINARY(16) NULL;
-- 部署兼容新旧列的应用，分批回填
UPDATE projects SET owner_id = :default WHERE id > :cursor LIMIT 1000;
-- 校验无 NULL 后再增加约束
```
...
种子数据和业务数据分开最终要落实到可观察证据。MySQL的配置、日志或执行结果需要携带稳定标识，例如 requestId、资源版本、任务 ID 或制品摘要，避免只凭“看起来正常”判断系统。
...
## expand/contract 保持版本兼容之后还要追问

### 迁移是有顺序的数据变更为什么不能只放在调用方处理？

调用方可以改善交互，但无法控制并发请求、绕过客户端的调用和进程故障。约束必须由拥有业务状态的一层执行，并由数据库约束、消息确认或运行时所有权提供最终裁决。

### expand/contract 保持版本兼容遇到超时后，什么时候可以重试？
...
### 种子数据和业务数据分开与前一层的责任怎样交接？

交接内容写入契约：输入带身份、范围和版本，输出带结果、状态码和可追踪 ID。种子数据和业务数据分开只处理自己拥有的状态。
