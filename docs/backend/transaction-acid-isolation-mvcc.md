...
事务把一组数据库读写绑定成一次提交或回滚；隔离级别决定并发事务彼此能看到什么；MVCC 用行版本支持快照读取。它们位于数据库存储引擎与业务不变量之间，数据库提供机制，应用仍需用条件更新、锁或版本字段表达“库存不能被重复消费”。
...
实验使用 MySQL 8.4、InnoDB 和两个客户端会话观察提交、回滚、快照读、当前读与并发更新，并且只应运行在隔离数据库。
...
## 机制复核：MySQL 事务、隔离级别与 MVCC：两个请求同时修改时发生了什么
