...
全文检索会分词并计算文本相关性，适合产品名、错误码和精确短语。中文效果依赖实际分词与词典，必须用代表性查询验证。向量检索把问题与 Chunk 映射到同一 Embedding 空间，再按余弦距离等度量排序；模型、维度和归一化策略要随记录保存，不能混用。
...
相同得分使用稳定 ID 打破平局，使同一输入得到可复现顺序。融合后可以对少量候选重排，但重排器只能看到已经授权的内容；超时后降级到 RRF，不能扩大到未过滤的全库候选。
