...
后端把 `nextCursor` 改成 `next_cursor` 后编译仍通过，React 列表却再也无法翻页。契约生成的价值，是在变更进入运行环境前把这种漂移变成明确失败。

## OpenAPI 同时描述协议和数据

OpenAPI 同时描述协议和数据不能只靠术语记忆。先确定输入来自谁、状态由谁拥有、一次操作改变了哪些记录，再看输出如何被下一层使用。**状态所有者一旦含糊，重试和故障恢复就会出现重复或丢失。**

```mermaid
flowchart LR
  A[输入与上下文] --> B[OpenAPI 同时描述协议和数据]
  B --> C[状态检查]
  C -->|满足约束| D[提交结果]
  C -->|不满足| E[稳定错误]
  D --> F[日志 / 指标 / 审计]
...
从契约生成客户端，而不是复制类型要放回请求时间线：开始时读到什么，中间获得什么锁、连接或租约，成功时提交什么，失败时又能否回到原状态。这样才能判断超时后是安全重试、查询原结果，还是进入人工对账。
...
兼容变更和契约门禁最终要落实到可观察证据。API 契约的配置、日志或执行结果需要携带稳定标识，例如 requestId、资源版本、任务 ID 或制品摘要，避免只凭“看起来正常”判断系统。
...
## 从契约生成客户端，而不是复制类型之后还要追问

### OpenAPI 同时描述协议和数据为什么不能只放在调用方处理？

调用方可以改善交互，但无法控制并发请求、绕过客户端的调用和进程故障。约束必须由拥有业务状态的一层执行，并由数据库约束、消息确认或运行时所有权提供最终裁决。

### 从契约生成客户端，而不是复制类型遇到超时后，什么时候可以重试？
...
### 兼容变更和契约门禁与前一层的责任怎样交接？

交接内容写入契约：输入带身份、范围和版本，输出带结果、状态码和可追踪 ID。兼容变更和契约门禁只处理自己拥有的状态。
