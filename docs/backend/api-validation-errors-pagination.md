...
请求校验、错误结构和游标分页分别负责三件事：在副作用前拒绝不合法输入，把失败转换成客户端可定位的协议，以及在数据变化时稳定地读取下一页。它们位于 API 边界和数据库查询之间，前端依赖前两者判断是否可提交，列表查询依赖游标保存排序位置。
...
这张图把错误放回发生位置：格式错误不应等数据库连接，权限拒绝不应泄露资源存在性，游标错误也不能靠前端猜 detail 文案。后面的代码和 SQL 分别验证这些交接点。
