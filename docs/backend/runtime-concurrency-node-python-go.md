...
并发运行时决定一个进程怎样安排重叠等待、CPU 工作、取消和资源上限；它位于 HTTP Handler 与数据库、HTTP 客户端、任务队列之间。Node.js、Python ASGI 和 Go 使用的执行单元不同，但都要回答在途任务由谁拥有、何时让出、何时停止以及连接池能承受多少工作。
