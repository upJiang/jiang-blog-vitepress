...
企业后台综合项目是围绕租户、权限、业务资源、文件和异步任务的一条可运行业务链；React 是客户端，三套后端实现协议，MySQL 保存业务事实，Redis、消息和对象存储分别承担短状态、任务投递和文件字节。它位于前面各专题的交汇处，用共同契约验证跨服务行为，而不是把工具清单全部启动。

同一个 React 管理端把 API Origin 从 NestJS 切到 FastAPI 或 Gin 后，登录、分页、权限、文件和任务进度应保持相同。综合项目用一份 OpenAPI、一套 MySQL 不变量和同一测试集合约束三套实现。
