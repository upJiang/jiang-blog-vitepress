...
缓存旁路模式通常先查 Redis，未命中再查 MySQL并回填。缓存与数据库是两次独立写入，所以必须接受短暂不一致，设计删除、过期、重建和穿透保护。Redis 分布式锁更准确地说是带过期时间的租约；持有者若停顿过久，锁可能过期，后续写入还需要 fencing token 或数据库条件保护。
...
## Docker、CI/CD 和 Kubernetes负责怎样运行代码
...
前五组先建立“请求怎样到达进程、数据怎样保存”的基础。读到这里，应该能把浏览器的一次请求和数据库的一条记录放在同一条因果链上。
