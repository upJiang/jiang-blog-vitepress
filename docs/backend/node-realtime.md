...
后台任务每次状态变化先在数据库写入递增序号事件，再发布通知。SSE 路由校验用户与任务所有权，从 `Last-Event-ID` 后查询历史事件，按序发送；Redis 或进程内通知只负责唤醒连接，不作为可重放真相源。

| 事件 | 载荷建议 | 客户端动作 |
| --- | --- | --- |
| `task.started` | 序号、时间 | 展示运行中 |
| `task.progress` | 已完成阶段，不含敏感正文 | 更新进度，游标前进 |
| `task.completed` | 结果引用 | 展示结果并关闭流 |
| `task.failed` | 稳定错误码 | 展示重试或联系入口 |

客户端只在事件成功应用后保存游标，重复序号直接忽略。页面从后台恢复时先读快照，再接收增量；游标早于保留窗口时，服务端返回最新快照并建立新起点。慢消费者采用连接缓冲上限，不能让单个浏览器拖住全局事件生产。

先用 SSE 解决服务器单向推送。只有客户端也需要高频主动发送、房间广播或二进制数据时再引入 WebSocket，并补上应用层心跳、认证续期、消息确认和背压。无论协议如何，客户端断线不改变后台任务状态。
