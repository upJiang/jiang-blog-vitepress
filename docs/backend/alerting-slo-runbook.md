...
CPU 80% 告警每天响几十次，但用户请求一直正常；真正的登录错误率上升却没人收到通知。告警应绑定用户可感知 SLI 和可执行动作，而不是监控所有波动。

## SLI 是测量，SLO 是目标

SLI 是测量，SLO 是目标不能只靠术语记忆。先确定输入来自谁、状态由谁拥有、一次操作改变了哪些记录，再看输出如何被下一层使用。**状态所有者一旦含糊，重试和故障恢复就会出现重复或丢失。**

```mermaid
flowchart LR
  A[输入与上下文] --> B[SLI 是测量，SLO 是目标]
  B --> C[状态检查]
  C -->|满足约束| D[提交结果]
  C -->|不满足| E[稳定错误]
  D --> F[日志 / 指标 / 审计]
...
错误预算连接可靠性和发布要放回请求时间线：开始时读到什么，中间获得什么锁、连接或租约，成功时提交什么，失败时又能否回到原状态。这样才能判断超时后是安全重试、查询原结果，还是进入人工对账。

下面的片段抓住 观测 中最容易出错的一条执行路径。先观察输入条件和状态标识，再看副作用在什么位置发生。

```text
SLI = 成功且低于 500ms 的项目列表请求 / 全部有效项目列表请求
SLO = 30 天窗口内 99.9%
告警 = 1h 和 6h 两个 burn rate 同时超阈值
```
...
Runbook 把告警变成行动最终要落实到可观察证据。观测的配置、日志或执行结果需要携带稳定标识，例如 requestId、资源版本、任务 ID 或制品摘要，避免只凭“看起来正常”判断系统。
...
### Runbook 把告警变成行动与前一层的责任怎样交接？

交接内容写入契约：输入带身份、范围和版本，输出带结果、状态码和可追踪 ID。Runbook 把告警变成行动只处理自己拥有的状态。
