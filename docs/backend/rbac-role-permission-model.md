...
页面隐藏了“删除用户”按钮，但攻击者仍可手写 DELETE 请求。RBAC 的授权判断必须发生在服务端，前端权限只改善界面，不构成安全边界。

## 用户、角色和权限形成多对多关系

用户、角色和权限形成多对多关系不能只靠术语记忆。先确定输入来自谁、状态由谁拥有、一次操作改变了哪些记录，再看输出如何被下一层使用。**状态所有者一旦含糊，重试和故障恢复就会出现重复或丢失。**

```mermaid
flowchart LR
  A[输入与上下文] --> B[用户、角色和权限形成多对多关系]
  B --> C[状态检查]
  C -->|满足约束| D[提交结果]
  C -->|不满足| E[稳定错误]
  D --> F[日志 / 指标 / 审计]
...
授权点应接近受保护动作要放回请求时间线：开始时读到什么，中间获得什么锁、连接或租约，成功时提交什么，失败时又能否回到原状态。这样才能判断超时后是安全重试、查询原结果，还是进入人工对账。
...
缓存失效和权限审计最终要落实到可观察证据。认证授权的配置、日志或执行结果需要携带稳定标识，例如 requestId、资源版本、任务 ID 或制品摘要，避免只凭“看起来正常”判断系统。
...
### 缓存失效和权限审计与前一层的责任怎样交接？

交接内容写入契约：输入带身份、范围和版本，输出带结果、状态码和可追踪 ID。缓存失效和权限审计只处理自己拥有的状态。
