...
ios 根据用户的行为来判定操作是不是属于流氓操作,如果是用户自己的动作触发的 window.open 就不会被阻止，比如写在 onclick 事件中，但如果是代码自动触发就会被阻止的。

解决方案，使用 window.location.href 跳转（在微信上 ios 好像还是有问题，建议增加弹窗确认跳转），或者为事件添加点击事件
