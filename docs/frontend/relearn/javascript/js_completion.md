...
description: "从规范内部记录理解 return、throw、break 和 continue"
category: frontend
...
Completion Record 是规范描述控制流的抽象，能解释 return、throw、break、continue 如何穿过语句和 finally。工程排障不必直接实现规范记录，但理解“异常会在哪一层被捕获、finally 是否覆盖返回值”有助于避免资源清理和错误处理的隐蔽 bug。
