---
title: "作用域与闭包"
description: "从词法环境和生命周期理解闭包的能力与成本"
category: frontend
...
闭包保存的是词法环境的可达绑定，不是“复制一份变量”。长生命周期监听器、定时器和缓存可能意外保留 DOM 或大对象；需要释放时提供 unsubscribe、AbortSignal 或明确的失效策略。闭包适合封装不变量，但不应隐藏关键状态迁移。
