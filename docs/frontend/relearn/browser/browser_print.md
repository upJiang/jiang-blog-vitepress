---
title: "浏览器绘制与合成"
description: "理解 paint、layer、raster 和 compositor"
category: frontend
...
我们讲解了浏览器中的位图操作部分，这包括了**渲染、合成和绘制**三个部分。渲染过程把元素变成位图，合成把一部分位图变成合成层，最终的绘制过程把合成层显示到屏幕上。
...
绘制和栅格化会受到图层边界、设备像素比、字体和图片解码影响。合成层不是越多越好；过度提升会增加内存和上传成本。验证要覆盖滚动、动画、缩放、低端设备和打印/截图场景，确保视觉内容与交互语义一致。
