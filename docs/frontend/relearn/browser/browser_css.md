...
样式计算、布局、绘制和合成不是每次更新都完整执行。改变几何属性可能触发布局，改变颜色通常只需绘制，transform 可能在合成阶段处理，但具体结果取决于元素、浏览器和资源。用 Performance 记录验证，不要把 will-change 当作通用加速开关。
