---
title: "DOM API 与节点操作"
description: "掌握节点、遍历、Range 和变更观察"
category: frontend
...
DOM API 修改节点时要考虑所有权、事件监听器、Range、MutationObserver 和自定义元素生命周期。批量插入可使用 DocumentFragment，但最终布局和可访问性更新仍需验证。观察器回调也在任务调度中执行，必须断开连接并限制处理量。
