...
SPA 生成高熵 `code_verifier`，计算 `S256` 得 `code_challenge`，保存 verifier 与一次性 state/nonce 的关联；授权请求只携带 challenge、client_id、redirect_uri、scope 和 state。

回调先验证 state，再把 code + verifier 通过 TLS 发送 Token Endpoint。授权服务器验证 challenge 后返回 Access Token，OIDC 场景同时返回并校验 ID Token 的签名、issuer、audience、nonce 和时间。
```text
