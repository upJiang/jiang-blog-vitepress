...
React Native 的 JS/React 树通过 JSI/Fabric 等机制把 props、events 和更新交给原生视图树，具体线程与架构按 RN 版本核对；JS 长任务仍会影响业务逻辑和桥接响应。Flutter 的 Dart Widget 经 Element、RenderObject 计算布局绘制，由引擎控制像素；平台插件在 Dart/native 边界传递消息。

Electron Renderer 是 Chromium 文档，Main Process 管窗口、文件和系统能力，preload 是受控隔离层。
```text
