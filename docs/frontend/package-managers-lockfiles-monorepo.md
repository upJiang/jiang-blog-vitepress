...
Node 安装包自带 npm。Yarn 与 pnpm 可以由 **Corepack** 管理，具体支持范围会随 Node 版本变化；Corepack 随 Node 14.19 到 24 分发，Node 25 起需要单独安装。

先运行 `corepack --version`，命令不存在时按 [Corepack 官方仓库](https://github.com/nodejs/corepack#how-to-install)安装，再参考 [Yarn 安装文档](https://yarnpkg.com/getting-started/install)和 [pnpm 安装文档](https://pnpm.io/installation)启用对应工具：
```bash
corepack enable
