...
CSS 不总是阻塞 DOM 解析，却会影响渲染和脚本读取样式；浏览器需要 DOM 与 CSSOM 计算样式。脚本、样式和预加载之间的实际优先级应从网络瀑布验证。
...
DOM 改动不必每次完整执行所有阶段，浏览器会失效并批量处理。读取布局后立刻写样式、循环重复会迫使同步 Layout，形成 layout thrashing。
...
白屏先判断 HTML 是否到达、关键 CSS/JS 是否失败、脚本是否抛错、主线程是否长任务、根节点是否渲染。按层收集证据比一句“从 URL 到页面”更能迁移到工作。

## 导航请求的进程与线程边界

地址栏提交后，浏览器先判定同文档导航、Service Worker 或网络导航；网络服务解析代理、DNS、连接和缓存。响应通过 MIME/CSP/下载判定后交给 Renderer，HTML tokenizer 与 tree builder 增量构建 DOM。遇到阻塞脚本会暂停解析，等待脚本获取和执行；CSSOM 是否完成又会影响脚本读取样式与首次绘制。
...
预加载扫描器可以提前发现后续资源，但动态拼接 URL、CSS import 和脚本依赖仍可能形成瀑布。布局树不等于 DOM 树，display:none、伪元素和匿名盒会改变几何；绘制列表与合成层也不是“一元素一层”。

## 安全决策位于链路中间

重定向每跳都可能改变 origin、凭证和 Referrer；HTTPS 证书、HSTS、mixed content、CSP、CORP/COEP 会在内容执行前阻断。页面看到 `200` 仍可能因为 MIME、CSP 或模块 CORS 无法执行脚本。排查必须同时看 Network response、Console security error 和 initiator。
