...
设计模式是对一类重复依赖关系的命名和结构选择，不是必须照抄的类模板。EventEmitter 是其中一种事件分发工具：发布者按事件名发送数据，订阅者注册回调，它位于产生事件的模块与响应事件的模块之间，用来减少双方的直接引用。这个间接层也会隐藏来源、顺序和生命周期，所以只有变化点清楚时才值得使用。
...
## 迁移复核：设计模式与 EventEmitter：从变化点选择结构
