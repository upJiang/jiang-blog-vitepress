...
Cookie 使用 Secure、HttpOnly、合适 SameSite 和最小 Domain/Path。前端无法通过 JavaScript设置 HttpOnly。XSS 可能以用户身份发请求，即使读不到 Cookie，所以仍需 CSP 与输入输出安全。
...
面试回答前端安全时，应把浏览器缓解与服务端最终授权分开，说明每层失败后下一层如何限制损失。
