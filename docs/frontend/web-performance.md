...
性能优化不是把懒加载、CDN、骨架屏、WebP 和防抖依次加到项目里。成熟的性能工程从用户任务出发：定义可观察指标，按设备/网络/页面分段，建立瓶颈假设，用浏览器证据定位，实施最小变化，再用真实用户数据和回归门禁证明收益没有转移成本。

一条“首屏 1.2 秒”的实验室数字不能代表真实用户，也不能说明交互是否可用。平均值会掩盖长尾；漂亮的骨架屏可能只遮住更慢的数据；Bundle 体积下降可能伴随更多异步瀑布。性能结论必须写清环境、分位数、样本和业务路径。
...
分片改善可响应性但增加总调度成本，并不能用于必须原子完成的计算。`requestIdleCallback` 支持与触发不稳定，不适合作为关键任务保证；`scheduler.yield/postTask` 等能力要按兼容性渐进增强。真正昂贵的哈希、解析、图布局可放 Worker，并使用 transferable 降低复制成本。
...
读写布局交错会强制同步 layout：循环中读取 `offsetWidth` 再写 style，浏览器反复刷新。批量读、计算、再写，并减少 DOM 数量。虚拟列表要处理动态高度、焦点、键盘、读屏与滚动锚定，不能只渲染可见行就算完成。
