...
**为什么在 Vite 中能够彻底解决低版本浏览器的兼容性问题，以及通过什么手段解决，需要借助哪些 JS 的工具和生态？**<br> `@babel/preset-env`、`core-js`、`regenerator-runtime`等等工具和基础库的强强联合，官方的 `Vite` 插件`@vitejs/plugin-legacy`将这些底层的工具链接入到 `Vite` 中，并实现开箱即用。
