...
当客户端收到连接同意的应答后，还要向服务端发送一个`确认报文`。客户端发完这个报文
...
发送端窗口是由接收窗口剩余大小决定的。接收方会把当前接收窗口的剩余大小写入应答报
...
Get 多用于无副作用，幂等的场景，例如搜索关键字。Post 多用于副作用，不幂等的场景
，例如注册。
...
并且 QUIC 在移动端的表现也会比 TCP 好。因为 TCP 是基于 IP 和端口去识别连接的，这
