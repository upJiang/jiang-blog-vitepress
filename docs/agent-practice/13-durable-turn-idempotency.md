...
客户端发送创建请求后网络超时，于是使用相同幂等键重试。第一次请求已经在数据库提交回合，第二次请求应读取并返回同一个回合 ID，而不是创建两个任务。幂等键需要绑定用户、操作类型和规范化请求摘要，避免不同业务意图误复用。

| 时间 | 动作 | 数据库事实 |
| --- | --- | --- |
| T1 | 第一次创建 | 写入 `pending` 回合和幂等身份 |
| T2 | 事务提交 | 回合可查询 |
| T3 | 派发 Celery | 消息可能成功，也可能失败 |
| T4 | 客户端重试 | 返回 T1 的回合 |
| T5 | Worker 竞争 | 只有持有 owner lease 的执行者推进 |

这套实践采用“数据库提交后派发 Celery”。它当前没有 transactional outbox，因此必须正视一个窗口：数据库已提交但消息派发失败。处理方式是把派发错误记录到回合，允许受控补派或由停滞扫描恢复；不能假装数据库和消息代理形成了同一事务。

准入控制分别限制全局运行量和单用户运行量，避免一个用户耗尽 Worker。owner lease 带所有者与到期时间，旧 Worker 在提交节点结果前再次确认租约。测试应同时发送两个相同幂等键、让第一次派发失败、再让两个 Worker 竞争，最后断言只有一个业务回合和一个有效执行所有者。
