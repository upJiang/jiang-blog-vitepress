...
选择需要 Checkpoint 的深入研究模式，让流程完成“理解”和第一轮检索后主动终止 Worker。此时数据库回合仍是 `running`，Checkpoint 保存的是稳定图状态，事件序列保存已经对客户端公开的事实。新的 Worker 接管前先验证 owner lease、原始 Deadline 和取消标记。

| 恢复步骤 | 需要确认 |
| --- | --- |
| 找到停滞回合 | 租约到期且尚未进入终态 |
| 决定恢复或终止 | 原 Deadline 未到、未被用户取消 |
| 加载 Checkpoint | 版本兼容，状态属于同一回合 |
| 重置流式通道 | 不重复发送已经持久化的事件 |
| 继续执行 | 从稳定节点开始，旧 Worker 无权提交 |

恢复不应重新获得完整时间预算。若创建时的绝对 Deadline 已到，扫描器直接写入超时终态；若用户已取消，恢复流程也不重新启动工具。每个长节点内部协作检查取消和 Deadline，不能只在节点之间检查。

测试时让旧 Worker 在新 Worker 接管后迟到返回。旧 Worker 提交前发现租约所有者变化，应丢弃结果；事件序列中只出现一个终态。短 Session 问答没有暂停价值时不启用 Checkpoint，减少序列化与存储负担。
