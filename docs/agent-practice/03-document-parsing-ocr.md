...
准备三页匿名 PDF：第一页是可复制文字，第二页只有扫描图片，第三页既有文本层又有一张表格。解析前先手工记录每页应该出现的标题、正文行和表头，这份记录就是最小真值。没有真值，只看到“解析成功”无法判断内容是否丢失。

普通解析先逐页运行。第一页应直接得到文本；第二页文本为空或低于可用阈值，才进入 OCR；第三页已有可用文本，不应因为图片存在就整页 OCR。OCR 的目标是补缺页，不是覆盖解析器已经保留的结构。

| 观察项 | 预期结果 | 异常时先查 |
| --- | --- | --- |
| 页码映射 | 输出还能定位原页 | 解析器是否在合并时丢页标识 |
| 扫描页 | 进入 OCR 并标记来源 | OCR 是否启用、语言包是否可用 |
| 文本页 | 不重复 OCR | 缺失页判定是否过宽 |
| 表格 | 保留行列或结构告警 | 文件类型适配器是否支持表格 |
| OCR 失败 | 整个导入明确失败或待处理 | 是否把空字符串当成功 |

运行结果应包含内容、页级状态和告警，而不只是一个大字符串。测试可以让 OCR 适配器主动抛错，确认任务不会发布空知识；再重复解析同一文件，确认来源哈希和解析版本能识别重复工作。

本篇边界是“把不同格式变成统一解析结果”。怎样切片、怎样发布版本留给后面两篇。解析器支持的格式也应来自真实适配器列表，未知格式要明确拒绝，不能用扩展名猜测成功。
