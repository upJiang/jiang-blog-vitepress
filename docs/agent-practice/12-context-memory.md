...
假设模型窗口中可用于应用输入的预算为 12,000 Token。先预留系统规则和回答空间，再把剩余部分分给当前问题、近期消息、摘要、长期记忆和检索证据。这里的数字只用于演示，真实值应按模型窗口和 Eval 调整。

| 内容 | 示例预算 | 超出时怎样处理 |
| --- | ---: | --- |
| 当前问题与范围 | 800 | 不裁掉关键约束，过长则请求聚焦 |
| 近期消息 | 2,000 | 按完整轮次从旧到新裁剪 |
| 滚动摘要 | 1,000 | 使用结构化短摘要 |
| 显式记忆 | 400 | 只选当前任务相关条目 |
| 检索证据 | 5,000 | 按证据预算和覆盖率选择 |

用十轮模拟对话验证：前两轮确定“只讨论访客账号”，中间更换话题，最后问“它有效多久”。近期消息与摘要应让系统解析“它”的焦点；如果摘要服务失败，确定性裁剪仍应保留当前问题和最近完整轮次，而不是随机截断字符串。

长期记忆只保存用户明确选择的稳定偏好，并提供查看与删除。一次性凭证、第三方隐私和模型推断不进入长期层。测试删除后再次构建上下文，确认条目不再出现；同时检查任何记忆都不能扩大当前 ACL 或覆盖系统规则。
