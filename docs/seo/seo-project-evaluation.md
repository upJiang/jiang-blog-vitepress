---
title: 创业网站是否值得投入 SEO
description: 在写内容和投广告前，判断需求、竞争、交付能力、现金流和测量条件。
category: seo
part: 第一部分：战略与证据
chapter: 2
tags:
...
上一篇完成的[搜索增长诊断表](/docs/seo/search-growth-model)是本篇输入。至少带上目标用户、核心转化、单次转化毛利、可投入预算和当前数据缺口；缺少这些字段时，任何项目评分都只能是低置信度假设。
...
假设要评估“面向小团队的预约工具”是否值得投入 SEO。不要先填一个总分，先为需求、商业价值、竞争入口、内容供给、技术交付和风险周期分别打 1–5 分，并在分数旁写证据。例如“需求 4 分”的证据可以是访谈原话、搜索词报告和结果页类型；“大家都需要”不算证据。
...
评分之后必须留下原始证据和不确定项。下一篇会把这些内容转成统一的证据强度、诊断状态和优先级，避免后续看到一个 SEO 分数就误以为已经得到了搜索引擎结论。
...
- 上一篇：[SEO、SEM、GEO 与搜索增长全景](/docs/seo/search-growth-model)
- 下一篇：[SEO 诊断的证据、评分与边界](/docs/seo/seo-evidence-scoring-boundaries)
