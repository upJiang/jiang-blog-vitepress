...
页面规划不是把每组词变成一篇文章。对每个需求组填写目标 URL、页面类型、主要承诺、辅助问题、转化动作、现有表现和处理状态。处理状态至少包含“更新、创建、合并、保留、暂不创建”，这样团队不会把页面数量当成果。

| 需求组 | 页面类型 | 决策 | 判断依据 |
| --- | --- | --- | --- |
| 预约工具价格 | 价格/方案页 | 更新 | 已有页面能承接，但缺少套餐条件 |
| 如何减少爽约 | 指南页 | 创建 | 独立学习意图，有可执行方法 |
| 某城市预约系统 | 地域页 | 暂不创建 | 没有真实本地差异与交付证据 |
| 两篇重复教程 | 一个主页面 | 合并 | 目标与结果页高度重叠 |

诊断关键词内耗时同时看查询、页面、时间和转化。两个 URL 偶尔都有展现不等于内耗；如果它们长期争夺相同查询、内容承诺相同且互相波动，再选择一个主页面，合并独有内容，更新内链与站点地图，并观察迁移结果。

程序化页面需要独特数据、真实需求、业务承接、规范 URL 和维护责任。先发布 20–50 个代表样本，抽查空值、重复标题、正文价值、状态码、内链与转化，再决定是否扩大。没有独特内容时，“暂不创建”是正确规划结果。
