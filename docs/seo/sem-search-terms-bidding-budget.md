...
description: 从真实搜索词管理广泛匹配、否定词、出价、预算和学习期，避免无证据调价。
category: seo
part: 第七部分：SEM 与协同增长
chapter: 19
tags:
...
# 关键词写的是“企业预约系统”，钱为什么花在“免费日历模板”上

广告主设置了一个看似高意图关键词，平台却根据匹配机制把广告展示给更多相关查询。报表里“企业预约系统”转化不错，但真实搜索词中出现大量免费模板、个人日历和竞品登录。只看关键词汇总，会把完全不同的需求平均在一起。

上一篇的[账户与意图结构图](/docs/seo/sem-account-keywords-landing)规定每个广告组要承接什么任务。本篇使用真实搜索词、业务价值和预算上限验证流量是否仍在边界内，并选择匹配与出价策略。输出不是一张“自动否词”列表，而是一套需要人工复核的治理规则。
...
验收至少包含一个应否定、一个因延迟需要观察、一个可扩展和一个证据不足候选。下一篇会进一步验收广告创意、实际最终页面、转化事件与业务归因。

## 继续学习

- 上一篇：[SEM 基础、账户结构与落地页](/docs/seo/sem-account-keywords-landing)
- 下一篇：[广告创意、落地页与转化追踪](/docs/seo/sem-creative-landing-tracking)
- 业务数据基础：[搜索表现分析与 SEO 归因](/docs/seo/search-performance-attribution)
