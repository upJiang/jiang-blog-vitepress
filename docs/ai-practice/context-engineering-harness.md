...
上下文工程决定一次 Agent 运行读取哪些规则、证据、历史和工具结果；Harness Engineering 则建设包围 Agent 的任务入口、权限、工具、状态、测试和停止条件。两者位于模型与真实开发环境之间，用来让同类任务获得可重复输入和可验证终态，而不是只依赖一段越来越长的 Prompt。
...
**Harness 让失败能够被观察、限制和复现，并把证据留给下一次改进。**
