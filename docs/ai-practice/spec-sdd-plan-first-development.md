...
Spec 与 SDD 的执行顺序是“读取任务约束 → 收集仓库证据 → 选择能力 → 执行最小改动 → 验证并交付”。用户目标从入口进入，需求版本记录进度，可执行规格通过当前主题的校验条件后才会交付。
...
用户目标是这条链路的起点；来源缺失时，程序只能拒绝请求或采用明确记录的默认值，模型补出的字段不能继续驱动收集仓库证据。

规格复述需求会破坏需求版本与可执行规格之间的对应关系，排查时先保留原始输入摘要和发生阶段，再判断是否允许重试。

先写完整方案再读仓库影响决策或下游解释，不能和规格复述需求共用“重新调用一次模型”的处理方式。

Spec 与 SDD 完成时至少留下需求版本、决策、可执行规格和检查结论；只有最终文字，无法证明结果来自本次请求。

需求版本在单进程 Demo 中可以留在内存，跨进程、带权限或有副作用的任务则要持久化输入、状态转移和停止原因，不能依赖 Prompt 保存状态。

Spec 与 SDD 还要把业务条件写成状态条件：不变量何时就绪，可执行规格何时允许交付，规格复述需求发生后是否还能继续。

Spec 与 SDD 对外区分完成、部分完成和无法确认，三个状态若都包装成可执行规格，上层界面无法采用不同处理方式。

## Spec 与 SDD 适合解决什么

Spec 与 SDD 接收用户目标、现有系统事实、不变量、接口、风险、验收条件和未知项，向下游交付可执行规格、任务顺序、验收映射、明确假设和更新记录，输入与输出之间用明确契约连接，调用方不能把一句含糊目标直接交给底层适配器。
...
依赖若只保存在 SDK 对象里，重试或进程退出后就失去解释依据；需要恢复或审计的字段进入持久层，体积较大的可执行规格只保存稳定引用。
...
状态合同中的需求版本表示当前进度，决策保存本次执行依赖的快照，依赖关联前后动作，三者不能合并成一段自由文本。
...
Spec 与 SDD 按“规格先记录必须保持的事实与可验证结果，再描述最小设计和任务依赖；实施中发现新事实时更新规格，而不是让计划与代码分叉”推进，这段机制必须落成需求版本和决策的实际变化，不能只列框架节点名称。
...
Spec 与 SDD 按输入、状态、依赖、结果和交付五个位置分类错误，发生位置决定恢复动作，不能用同一个重试循环处理全部失败。
...
Spec 与 SDD 结束错误处理时保存终态、可重试性和资源清理结果，后台扫描器依靠需求版本和这些字段区分仍在运行、等待恢复和已经失败的任务。

Spec 与 SDD 在执行前写入绝对 Deadline、最大动作数、重复签名、无进展阈值、预算和人工取消信号，任一条件触发后选择能力都不再创建新工作。
...
Spec 与 SDD 的运行手册从 request_id 定位需求假设、文件定位、差异、命令输出、失败说明和回滚入口，再决定重试、补偿、取消或回滚，无需先展开完整的用户目标。
...
Spec 与 SDD 增加了需求版本、依赖调用、恢复责任和故障面，设计记录既要说明获得的能力，也要列出新增的退出、降级与回滚路径。

Spec 与 SDD 从一个调用、一个需求版本、一组明确错误和几条确定性测试开始，真实 Trace 证明需要后再加入并发、缓存、队列或更复杂策略。

PRD 只在能处理规格复述需求时引入，不为目录完整强行组合；接口保持可替换，后续需求出现再扩展。
