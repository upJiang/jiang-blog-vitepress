...
个人 AI 工作系统的执行顺序是“读取任务约束 → 收集仓库证据 → 选择能力 → 执行最小改动 → 验证并交付”。任务清单从入口进入，任务模式记录进度，可追踪任务通过当前主题的校验条件后才会交付。
...
任务清单是这条链路的起点；来源缺失时，程序只能拒绝请求或采用明确记录的默认值，模型补出的字段不能继续驱动收集仓库证据。

所有事情放一个长会话会破坏任务模式与可追踪任务之间的对应关系，排查时先保留原始输入摘要和发生阶段，再判断是否允许重试。

长期目标没有停止条件影响会话或下游解释，不能和所有事情放一个长会话共用“重新调用一次模型”的处理方式。

个人 AI 工作系统完成时至少留下任务模式、会话、可追踪任务和检查结论；只有最终文字，无法证明结果来自本次请求。

任务模式在单进程 Demo 中可以留在内存，跨进程、带权限或有副作用的任务则要持久化输入、状态转移和停止原因，不能依赖 Prompt 保存状态。

个人 AI 工作系统还要把业务条件写成状态条件：不确定性何时就绪，可追踪任务何时允许交付，所有事情放一个长会话发生后是否还能继续。

个人 AI 工作系统对外区分完成、部分完成和无法确认，三个状态若都包装成可追踪任务，上层界面无法采用不同处理方式。

## 个人 AI 工作系统适合解决什么

个人 AI 工作系统接收任务清单、目标周期、不确定性、数据敏感度、可用工具、精力预算和交付责任，向下游交付可追踪任务、验证过的产物、明确权限记录、复盘结论和下一次可复用上下文，输入与输出之间用明确契约连接，调用方不能把一句含糊目标直接交给底层适配器。
...
长期目标若只保存在 SDK 对象里，重试或进程退出后就失去解释依据；需要恢复或审计的字段进入持久层，体积较大的可追踪任务只保存稳定引用。
...
状态合同中的任务模式表示当前进度，会话保存本次执行依赖的快照，长期目标关联前后动作，三者不能合并成一段自由文本。
...
个人 AI 工作系统按“短任务使用直接对话或工具，复杂任务先计划，持续目标单独保存状态；所有外部写入和发布按权限确认，完成后把可复用规则与证据沉淀”推进，这段机制必须落成任务模式和会话的实际变化，不能只列框架节点名称。
...
个人 AI 工作系统按输入、状态、依赖、结果和交付五个位置分类错误，发生位置决定恢复动作，不能用同一个重试循环处理全部失败。
...
个人 AI 工作系统结束错误处理时保存终态、可重试性和资源清理结果，后台扫描器依靠任务模式和这些字段区分仍在运行、等待恢复和已经失败的任务。

个人 AI 工作系统在执行前写入绝对 Deadline、最大动作数、重复签名、无进展阈值、预算和人工取消信号，任一条件触发后选择能力都不再创建新工作。
...
个人 AI 工作系统的运行手册从 request_id 定位需求假设、文件定位、差异、命令输出、失败说明和回滚入口，再决定重试、补偿、取消或回滚，无需先展开完整的任务清单。
...
个人 AI 工作系统增加了任务模式、依赖调用、恢复责任和故障面，设计记录既要说明获得的能力，也要列出新增的退出、降级与回滚路径。

个人 AI 工作系统从一个调用、一个任务模式、一组明确错误和几条确定性测试开始，真实 Trace 证明需要后再加入并发、缓存、队列或更复杂策略。

任务管理只在能处理所有事情放一个长会话时引入，不为目录完整强行组合；接口保持可替换，后续需求出现再扩展。
