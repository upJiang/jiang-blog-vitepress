...
AI Coding 交付链的执行顺序是“读取任务约束 → 收集仓库证据 → 选择能力 → 执行最小改动 → 验证并交付”。需求从入口进入，需求假设记录进度，聚焦的代码差异通过当前主题的校验条件后才会交付。
...
需求是这条链路的起点；来源缺失时，程序只能拒绝请求或采用明确记录的默认值，模型补出的字段不能继续驱动收集仓库证据。

跳过现状调查会破坏需求假设与聚焦的代码差异之间的对应关系，排查时先保留原始输入摘要和发生阶段，再判断是否允许重试。

越权提交或部署影响代码证据或下游解释，不能和跳过现状调查共用“重新调用一次模型”的处理方式。

AI Coding 交付链完成时至少留下需求假设、代码证据、聚焦的代码差异和检查结论；只有最终文字，无法证明结果来自本次请求。

需求假设在单进程 Demo 中可以留在内存，跨进程、带权限或有副作用的任务则要持久化输入、状态转移和停止原因，不能依赖 Prompt 保存状态。

AI Coding 交付链还要把业务条件写成状态条件：仓库约束何时就绪，聚焦的代码差异何时允许交付，跳过现状调查发生后是否还能继续。

AI Coding 交付链对外区分完成、部分完成和无法确认，三个状态若都包装成聚焦的代码差异，上层界面无法采用不同处理方式。

## AI Coding 交付链适合解决什么

AI Coding 交付链接收需求、验收条件、仓库约束、现有代码、测试、运行环境、风险和发布授权，向下游交付聚焦的代码差异、可复现验证、候选制品、残余风险和回滚说明，输入与输出之间用明确契约连接，调用方不能把一句含糊目标直接交给底层适配器。
...
计划若只保存在 SDK 对象里，重试或进程退出后就失去解释依据；需要恢复或审计的字段进入持久层，体积较大的聚焦的代码差异只保存稳定引用。
...
状态合同中的需求假设表示当前进度，代码证据保存本次执行依赖的快照，计划关联前后动作，三者不能合并成一段自由文本。
...
AI Coding 交付链按“先读约束与现状，再确定最小影响面；实现后按风险运行测试、审查差异、构建候选并旁路验证，只有获得授权才发布”推进，这段机制必须落成需求假设和代码证据的实际变化，不能只列框架节点名称。
...
AI Coding 交付链按输入、状态、依赖、结果和交付五个位置分类错误，发生位置决定恢复动作，不能用同一个重试循环处理全部失败。
...
AI Coding 交付链结束错误处理时保存终态、可重试性和资源清理结果，后台扫描器依靠需求假设和这些字段区分仍在运行、等待恢复和已经失败的任务。

AI Coding 交付链在执行前写入绝对 Deadline、最大动作数、重复签名、无进展阈值、预算和人工取消信号，任一条件触发后选择能力都不再创建新工作。
...
AI Coding 交付链的运行手册从 request_id 定位需求假设、文件定位、差异、命令输出、失败说明和回滚入口，再决定重试、补偿、取消或回滚，无需先展开完整的需求。
...
AI Coding 交付链增加了需求假设、依赖调用、恢复责任和故障面，设计记录既要说明获得的能力，也要列出新增的退出、降级与回滚路径。

AI Coding 交付链从一个调用、一个需求假设、一组明确错误和几条确定性测试开始，真实 Trace 证明需要后再加入并发、缓存、队列或更复杂策略。

Coding Agent 只在能处理跳过现状调查时引入，不为目录完整强行组合；接口保持可替换，后续需求出现再扩展。
