...
示例服务先验证工具描述、参数 Schema、身份范围和错误映射，再接入真实模型。内存注册表和 Fake Client 只能证明发现与调用顺序，不能证明网络传输、认证、并发、超时和供应商协议兼容。

发布前用只读工具覆盖合法参数、缺失参数、超出 Scope、重复请求和客户端断线。服务端日志保留调用 ID、耗时、结果类型和拒绝原因，敏感参数与完整上下文进入受控存储。
