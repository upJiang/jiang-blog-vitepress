...
实现文章检查 Skill 的执行顺序是“读取任务约束 → 收集仓库证据 → 选择能力 → 执行最小改动 → 验证并交付”。Markdown 文件从入口进入，解析结果记录进度，带文件位置的问题通过当前主题的校验条件后才会交付。
...
Markdown 文件是这条链路的起点；来源缺失时，程序只能拒绝请求或采用明确记录的默认值，模型补出的字段不能继续驱动收集仓库证据。

把关键词数量当质量会破坏解析结果与带文件位置的问题之间的对应关系，排查时先保留原始输入摘要和发生阶段，再判断是否允许重试。

自动删除未知段落影响规则命中或下游解释，不能和把关键词数量当质量共用“重新调用一次模型”的处理方式。

实现文章检查 Skill 完成时至少留下解析结果、规则命中、带文件位置的问题和检查结论；只有最终文字，无法证明结果来自本次请求。

解析结果在单进程 Demo 中可以留在内存，跨进程、带权限或有副作用的任务则要持久化输入、状态转移和停止原因，不能依赖 Prompt 保存状态。

实现文章检查 Skill 还要把业务条件写成状态条件：标题结构何时就绪，带文件位置的问题何时允许交付，把关键词数量当质量发生后是否还能继续。

实现文章检查 Skill 对外区分完成、部分完成和无法确认，三个状态若都包装成带文件位置的问题，上层界面无法采用不同处理方式。

## 实现文章检查 Skill 适合解决什么

实现文章检查 Skill 接收 Markdown 文件、Frontmatter 规范、标题结构、链接规则、隐私规则、目标读者和事实来源，向下游交付带文件位置的问题、可自动修复项、需作者判断项、复测结果和未通过原因，输入与输出之间用明确契约连接，调用方不能把一句含糊目标直接交给底层适配器。
...
人工判断项若只保存在 SDK 对象里，重试或进程退出后就失去解释依据；需要恢复或审计的字段进入持久层，体积较大的带文件位置的问题只保存稳定引用。
...
状态合同中的解析结果表示当前进度，规则命中保存本次执行依赖的快照，人工判断项关联前后动作，三者不能合并成一段自由文本。
...
实现文章检查 Skill 按“脚本负责唯一 H1、层级、链接、代码围栏和隐私模式等确定性检查，Skill 负责连贯性、事实边界和读者理解，二者输出统一问题位置”推进，这段机制必须落成解析结果和规则命中的实际变化，不能只列框架节点名称。
...
实现文章检查 Skill 按输入、状态、依赖、结果和交付五个位置分类错误，发生位置决定恢复动作，不能用同一个重试循环处理全部失败。
...
实现文章检查 Skill 结束错误处理时保存终态、可重试性和资源清理结果，后台扫描器依靠解析结果和这些字段区分仍在运行、等待恢复和已经失败的任务。

实现文章检查 Skill 在执行前写入绝对 Deadline、最大动作数、重复签名、无进展阈值、预算和人工取消信号，任一条件触发后选择能力都不再创建新工作。
...
实现文章检查 Skill 的运行手册从 request_id 定位需求假设、文件定位、差异、命令输出、失败说明和回滚入口，再决定重试、补偿、取消或回滚，无需先展开完整的 Markdown 文件。
...
实现文章检查 Skill 从一个调用、一个解析结果、一组明确错误和几条确定性测试开始，真实 Trace 证明需要后再加入并发、缓存、队列或更复杂策略。

Linter 只在能处理把关键词数量当质量时引入，不为目录完整强行组合；接口保持可替换，后续需求出现再扩展。
