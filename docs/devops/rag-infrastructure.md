...
RAG Infra 是支撑文档导入、版本发布、索引构建、权限过滤、检索和证据装配的基础设施。它横跨离线数据平面与在线查询平面，位于知识来源和模型上下文之间，用来让回答能够引用当前范围内的可追溯资料。向量相似度只是其中一个排序信号。
...
RAG（Retrieval-Augmented Generation）是“先从受约束的知识源检索，再把检索结果交给模型生成”的应用链。RAG Infra 位于对象存储、解析/向量索引和 Agent Runtime 之间：它负责把文件变成可发布的知识版本，也负责在查询前固定租户范围和版本，最终产出带来源的 Evidence。它不是一个向量数据库，也不替模型决定业务权限。

下面的内容沿着一份混合文档走完两条路径。任何阶段缺少稳定 ID、Scope 或 Release，后面的相似度和引用都无法补救。
...
一个最小的端到端 fixture 至少包含两份同名文档：`release=r1` 的旧版和 `release=r2` 的新版，再加入一个不同租户的同义片段。测试固定 `tenant=t1, release=r2`，依次断言：导入只激活完整版本；召回查询带 Scope/Release 条件；Evidence 包含文档版本和定位；答案不能引用 `r1` 或 `t2`。再把 `r2` 标记撤回，查询应得到空证据并拒答，而不是退回旧版本。

这组 fixture 比单独断言“向量相似度最高”更能说明 RAG 的职责，因为它同时覆盖发布、权限、版本、证据和无答案路径。

RAG Infra 交付的是一条可重建、可评测、可发布和可拒答的证据供应链，向量数据库只是其中一个存储与检索组件。
