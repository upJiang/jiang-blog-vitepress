...
没有 NVIDIA GPU 的当前环境不执行真实模型加载，因此本篇只定义验证证据。实际环境至少检查：文件完整、模型架构被引擎识别、Tokenizer 往返合理、Chat Template 生成预期角色、短输入可以完成、上下文上限被拒绝、停止条件正确、显存与日志没有异常。
