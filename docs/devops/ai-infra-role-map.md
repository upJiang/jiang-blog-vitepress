...
举一个具体例子：用户提交一条带文件的问答请求时，平台要确认文件版本已经发布，检索任务有可用的 Worker，模型实例拥有足够显存，网关知道这次请求属于哪个租户，回答结束后还要留下费用和引用记录。算法可以改变回答策略，但这些状态不会因为模型换成更大的版本就自动出现。AI Infra 负责把它们组织成一条能重复执行、能观察、能停止的运行链。

它和普通的服务器运维也有差别。普通 Web 服务往往以请求数和 CPU 为主要容量单位，AI 服务还会受到 Token 数、KV Cache、模型加载时间、知识版本和 GPU 拓扑影响。相同的 HTTP 代码，换一个上下文长度或模型精度，资源账本就可能完全不同。
...
例如一个企业问答请求先经过网关鉴权，再读取已发布的知识版本，最后进入拥有足够显存的模型实例。每个阶段都有不同输入和失败证据：权限拒绝不是模型错误，模型加载失败也不是用户网络错误。AI Infra 的作用就是把这些运行条件和状态交接固定下来，让请求能被重复执行、观测和恢复。

它与模型算法的区别在于责任对象。算法关心参数、损失和回答质量，Infra 关心制品从哪里来、资源如何分配、请求何时准入、失败如何止损。两者会互相影响，但不能用“模型分数高”证明服务容量足够，也不能用“服务稳定”证明回答正确。
...
从这张责任表继续学习，后面的主题会落到具体字段和命令。Linux 解释服务进程怎样读取配置、绑定端口和响应信号；网络解释请求怎样从域名走到 HTTP；容器和 Compose 解释进程如何隔离并组成服务栈；GPU 与 Kubernetes 解释计算资源怎样进入模型实例；Gateway、Agent 和 RAG 再把这些能力组织成企业平台。

## 机制复核：什么是 AI Infra？它为什么不只是部署模型
