...
LLM Serving 是把模型权重、Tokenizer、推理引擎和有限计算资源封装成在线生成接口的运行层。它位于应用或网关与 GPU 推理之间，负责模型加载、请求排队、批处理、流式输出和运行终态。端口可访问只证明 Web 进程在工作，不代表模型已经具备目标请求容量。
...
完整生命周期先确定各组件的输入、状态和故障传播，Transformer 推理、动态批处理、vLLM 和量化再落到对应阶段。
