...
“备份命令成功”只证明文件被写出，不证明它完整、可读或具备恢复所需密钥。为一次演练记录数据库版本、备份开始/结束时间、校验和、对象存储依赖、加密材料位置和预期 RPO/RTO，然后恢复到与生产隔离的环境。

| 恢复阶段 | 验收内容 |
| --- | --- |
| 创建备份 | 无错误、大小合理、校验和固定 |
| 隔离恢复 | Schema、行数和关键约束存在 |
| 应用启动 | 对应版本能通过只读健康与业务查询 |
| 一致性检查 | 数据库与对象引用、事件序号可核对 |
| 计时 | 实际数据丢失窗口与恢复时间 |

Schema 迁移采用 Expand-Migrate-Contract：先增加兼容字段，新旧代码都能运行；再分批回填并记录进度；应用切换稳定后才停止写旧字段；最后在独立发布中删除。长回填有检查点和速率限制，不把整表锁定与应用部署绑在一起。

时间点恢复后，外部邮件、支付或对象写入不会自动倒退，需要依据幂等身份和审计事实对账。演练产物包括命令、版本、耗时、差异、负责人和下一次日期。没有执行过隔离恢复的备份，只能被称为“备份文件”，还不能成为可靠回滚能力。
