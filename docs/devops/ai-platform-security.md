...
AI Platform 安全是对租户身份、Secret、知识数据、模型制品、工具权限和审计记录建立信任边界。它位于外部内容、Agent Runtime、模型服务和基础设施控制面之间，用来阻止提示注入越权、跨租户访问、凭证泄露和不受控出站。模型输出从来不是授权结果。
...
AI Platform 安全是对模型、数据、工具和运行资源的确定性边界管理。它位于 Gateway、RAG、Agent Runtime、Serving 与基础设施的交界处，回答“谁能读什么、模型能提出什么、程序允许执行什么、日志可以留下什么”。Prompt 只能表达意图，不能替代身份、授权、网络和审计。
...
最小回归可以用一份带恶意指令的文档串起来：文档进入查询范围后，模型候选请求 `delete_note`，策略只注册 `search_notes`，工具执行器记录 `decision=deny`；随后发起一个指向 `169.254.169.254` 的抓取请求，应在 DNS/地址校验前被拒绝；最后检查 Trace 和错误日志中没有 Authorization Header、原文 Secret 或完整 Prompt。三项都通过，才说明“模型输出不拥有权限”落到了可观察行为上。
