...
这张图里的 GET 与 SET 各自原子，整个 cache-aside 过程不是事务。读取数据库后到回填 Redis 前，记录可能又更新。版本号、失效通知和可接受陈旧窗口决定是否需要更严格方案，不能从图中推出缓存与数据库永远一致。
...
## 机制复核：Redis 是什么？缓存、Session、限流与队列该怎样选择
