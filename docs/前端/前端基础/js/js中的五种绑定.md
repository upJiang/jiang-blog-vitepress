...
> 当函数调用没有任何前缀的情况下，this 在非严格模式下指向 window，在严格模式下指向 undefined，在严格模式下调用不在严格模式下的函数，并不影响 this 的返回
...
new 执行了什么<br/> (1) 创建一个新对象<br/> (2) 将构造函数的作用域赋给新对象（因此 this 就指向了这个新对象） <br/> (3) 执行构造函数中的代码（为这个新对象添加属性） <br/> (4) 返回新对象<br/>
