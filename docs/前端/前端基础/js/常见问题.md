...
- `getBoundingClientRect()` `getBoundingClientRect()` 返回值是一个 DOMRect 对象，拥有`left, top, right, bottom, x, y, width, height`属性。公式: `el.getBoundingClientRect().top <= viewPortHeight`
